...
static void __proc_teen_notice(char *login, int hour)
{
	...
	switch (hour)
	{
		...
		case 2:
			{
				ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "당신의 누적 온라인 시간이 이미 %d시간이 지났습니다."), hour);
				SET_OVER_TIME(ch, OT_NONE);
			}
			break;
		case 3:
			{
				ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "당신의 누적 온라인 시간이 이미 %d시간이 되었습니다,"), hour);
				ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "조속히 접속을 종료하신 후 건강을 위해 휴식을 취해주시기바랍니다."));
				SET_OVER_TIME(ch, OT_3HOUR);
			}
			break;
		case 4:
			{
				ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "당신은 이미 게임 피로 상태에 들어섰으며,"));
				ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "게임 내의 수익이 정상치의 50%로 하향됩니다."));
				ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "당신의 건강을 위해 조속히 접속을 종료하시고"));
				ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "휴식 및 학업에 열중해주십시오."));
				SET_OVER_TIME(ch, OT_3HOUR);
			}
			break;
		default:
			{
				ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "당신은 이미 온전하지 못한 게임 시간에 들어섰습니다."));
				ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "건강을 위해 조속히 접속을 종료하신 후 휴식을 취해주십시오,"));
				ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "그렇지 않으면 건강 상에 피해를 입을 수 있으며 게임 내의 수치는 0이 됩니다."));
				ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "누적 오프라인 시간이 5시간이 되면 정상으로 돌아옵니다."));
...
static void __proc_login_notice(char *login, int on_time, int off_time)
{
	...
	if (0==on_time)
	{
		ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "당신의 누적 오프라인 시간은 %s입니다."), off_time_string);
		ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "당신의 누적 온라인 시간은 0이 되었습니다. 100% 효과를 얻으실 수 있습니다."));
		return;
	}
	else
	{
		ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "당신의 누적 오프라인 시간은 %s이며, 누적 온라인 시간은 %s입니다."),
