...
namespace quest
{
	...
	#error "lua version not found"
		...
		if (LC_IsEurope())
		{
			char translateDefaultFileName[256];
			snprintf(translateDefaultFileName, sizeof(translateDefaultFileName), "%s/multi_translate.lua", LocaleService_GetBasePath().c_str());
			if (lua_dofile(L, translateDefaultFileName) != 0)
			{
				sys_err("LOAD_TRANSLATE_DEFAULT_ERROR(%s)", translateDefaultFileName);
				return false;
			}
			...
			for (int i = 0; i < _countof(g_stLanguagesName); i++)
			{
				...
				snprintf(translateFileName, sizeof(translateFileName), "%s/translate/quest/translate_%s.lua", LocaleService_GetBasePath().c_str(), g_stLanguagesName[i].c_str());
				if (lua_dofile(L, translateFileName) != 0)
				{
