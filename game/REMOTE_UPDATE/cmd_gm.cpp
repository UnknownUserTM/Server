...
ACMD(do_announcement)
{
	...
	ch->ChatPacket(CHAT_TYPE_COMMAND, "BINARY_OpenAnnouncementWindow");
}
...
ACMD(do_state)
{
	...
	for (int i = 0; i < MAX_PRIV_NUM; ++i)
		if (CPrivManager::instance().GetPriv(tch, i))
		{
			...
			if (iByEmpire)
				ch->ChatPacket(CHAT_TYPE_INFO, "%s for empire : %d", LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), c_apszPrivNames[i]), iByEmpire);

			if (iByGuild)
				ch->ChatPacket(CHAT_TYPE_INFO, "%s for guild : %d", LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), c_apszPrivNames[i]), iByGuild);

			if (iByPlayer)
				ch->ChatPacket(CHAT_TYPE_INFO, "%s for player : %d", LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), c_apszPrivNames[i]), iByPlayer);
		}
}
...
struct notice_packet_func
{
	...
	void operator () (LPDESC d)
	{
		if (!d->GetCharacter())
			return;
#ifdef ENABLE_FULL_NOTICE
		d->GetCharacter()->ChatPacket((m_bBigFont) ? CHAT_TYPE_BIG_NOTICE : CHAT_TYPE_NOTICE, LC_TEXT_CONVERT_LANGUAGE(d->GetCharacter()->GetLanguage(), m_str));
#else
		d->GetCharacter()->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT_CONVERT_LANGUAGE(d->GetCharacter()->GetLanguage(), m_str));
#endif
	}
};
...
struct notice_map_packet_func
{
	...
	void operator() (LPDESC d)
	{
		if (!d->GetCharacter() || d->GetCharacter()->GetMapIndex() != m_mapIndex)
			return;
...
ACMD(do_set_stat)
{
	...
	else
	{
		...
		case POINT_HT:
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "Cannot set stat under initial stat."));
				return;
			}
			n = 0;
			break;
...
{
	ch->TEAM_SetServerleitungRank();
	ch->SetQuestFlag("gmpostboard.rang_index",1);
}

ACMD (do_rem_serverleitung_rang)
{
	ch->TEAM_RemServerleitungRank();
	ch->SetQuestFlag("gmpostboard.rang_index",0);
}

ACMD (do_set_teamleitung_rang)
{
	ch->TEAM_SetTeamleitungRank();
	ch->SetQuestFlag("gmpostboard.rang_index",2);
}

ACMD (do_rem_teamleitung_rang)
{
	ch->TEAM_RemTeamleitungRank();
	ch->SetQuestFlag("gmpostboard.rang_index",0);
}
...
{
	ch->TEAM_SetSGMRank();
	ch->SetQuestFlag("gmpostboard.rang_index",3);
}
...
{
	ch->TEAM_SetDevTecRank();
	ch->SetQuestFlag("gmpostboard.rang_index",4);
}

ACMD (do_rem_dev_tec_rang)
{
	ch->TEAM_RemDevTecRank();
	ch->SetQuestFlag("gmpostboard.rang_index",0);
}
...
{
	ch->TEAM_SetSUPRank();
	ch->SetQuestFlag("gmpostboard.rang_index",5);
}
...
{
	ch->TEAM_SetSERank();
	ch->SetQuestFlag("gmpostboard.rang_index",6);
}
...
// {
	// ch->TEAM_SetDeveloperRank();
	// ch->SetQuestFlag("gmpostboard.rang_index",1);
// }

// ACMD (do_rem_developer_rang)
// {
	// ch->TEAM_RemDeveloperRank();
	// ch->SetQuestFlag("gmpostboard.rang_index",0);
// }

// ACMD (do_set_organize_rang)
// {
	// ch->TEAM_SetOrganizeRank();
	// ch->SetQuestFlag("gmpostboard.rang_index",2);
// }

// ACMD (do_rem_organize_rang)
// {
	// ch->TEAM_RemOrganizeRank();
	// ch->SetQuestFlag("gmpostboard.rang_index",0);
// }

// ACMD (do_set_support_rang)
// {
	// ch->TEAM_SetSupportRank();
	// ch->SetQuestFlag("gmpostboard.rang_index",3);
// }

// ACMD (do_rem_support_rang)
// {
	// ch->TEAM_RemSupportRank();
	// ch->SetQuestFlag("gmpostboard.rang_index",0);
// }
...
ACMD(do_plague)
{
	...
	int amount = 0, timer = 0;
	if (isnhdigit(*arg1) && isnhdigit(*arg2))
	{
		str_to_number(amount, arg1);
		str_to_number(timer, arg2);
	}	
	int plague_event_timer = get_global_time() + (timer*60);
	quest::CQuestManager::instance().SetEventFlag("plague_event", plague_event_timer);	
	quest::CQuestManager::instance().SetEventFlag("plague_count", amount);	
	
	for (int i=0;i<amount;i++)
	{	
		LPCHARACTER tch = CHARACTER_MANAGER::instance().SpawnMobRange(6650,
				ch->GetMapIndex(),
				ch->GetX() - number(1250, 1750),
				ch->GetY() - number(1250, 1750),
				ch->GetX() + number(1250, 1750),
				ch->GetY() + number(1250, 1750),
				true,
				true);	
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "[PlagueEvent] gestartet! MonsterCount: %d, Zeit: %d Min. Time: %d", amount, timer, plague_event_timer);
	BroadcastNotice("Plagentrager sind in Pyungmoo erschienen! besiegt sie in 10 Min. oder die Seuche wird sich ausbreiten...");
}
