...
struct FuncShout
{
	...
	const char * m_strLanguage;
	FuncShout(const char * str, BYTE bEmpire, const char * strName, const char * strLanguage) : m_str(str), m_bEmpire(bEmpire), m_strName(strName), m_strLanguage(strLanguage)
	...
	void operator () (LPDESC d)
	{
		...
		if (!d->GetCharacter() || (d->GetCharacter()->GetGMLevel() == GM_PLAYER && d->GetEmpire() != m_bEmpire))
			return;
#endif
		d->GetCharacter()->FilterChatPacket(m_strName, m_strLanguage, m_bEmpire, m_str);
	}
};

void SendShout(const char * szText, BYTE bEmpire, const char * szName, const char * szLanguage)
{
	const DESC_MANAGER::DESC_SET & c_ref_set = DESC_MANAGER::instance().GetClientSet();
	std::for_each(c_ref_set.begin(), c_ref_set.end(), FuncShout(szText, bEmpire, szName, szLanguage));
}

void CInputP2P::Shout(const char * c_pData)
{
	TPacketGGShout * p = (TPacketGGShout *) c_pData;
	SendShout(p->szText, p->bEmpire, p->szName, p->szLanguage);
}
...
int CInputP2P::Analyze(LPDESC d, BYTE bHeader, const char * c_pData)
{
	...
	switch (bHeader)
	{
		...
		case HEADER_GG_ANNOUNCEMENT:
			CLanguageManager::instance().AnnouncementP2P(c_pData);
			break;	
	}
