...
LPITEM ITEM_MANAGER::CreateItem(DWORD vnum, DWORD count, DWORD id, bool bTryMagic, int iRarePct, bool bSkipSave)
{
	...
	if (id == 0) // »õ·Î ¸¸µå´Â ¾ÆÀÌÅÛÀÏ ¶§¸¸ Ã³¸®
	{
		...
		{
			int wert = number(1, 5);
			item->SetSocket(0, wert);
		}			
		
		// Pets
		if (vnum >= 55001 && vnum <= 55004)
		{
			item->SetSocket(0, 1); // Level
			item->SetSocket(1, 0); // MobEXP
			item->SetSocket(2, 0); // ItemEXP
			
			item->SetSocket(3, get_global_time()+(15*24*60*60));
			item->SetSocket(5, 1);
