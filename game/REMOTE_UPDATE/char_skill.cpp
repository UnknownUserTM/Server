...
bool CHARACTER::LearnGrandMasterSkill(DWORD dwSkillVnum)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "수련할 수 없는 스킬입니다."));
		return false;
	}
	...
	if (pkSk->dwType == 0)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "그랜드 마스터 수련을 할 수 없는 스킬입니다."));
		return false;
	}
	...
	{
		if (GetSkillMasterType(dwSkillVnum) > SKILL_GRAND_MASTER)
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "퍼펙트 마스터된 스킬입니다. 더 이상 수련 할 수 없습니다."));
		else
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "이 스킬은 아직 그랜드 마스터 수련을 할 경지에 이르지 않았습니다."));
		return false;
	}
	...
	if (FindAffect(AFFECT_SKILL_BOOK_BONUS))
	{
		...
		// YRemoveAffect(AFFECT_SKILL_BOOK_BONUS);
	}
	...
	{
		ChatPacket(CHAT_TYPE_TALKING, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "크윽, 기가 역류하고 있어! 이거 설마 주화입마인가!? 젠장!"));
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "수련이 실패로 끝났습니다. 다시 도전해주시기 바랍니다."));
		LogManager::instance().CharLog(this, dwSkillVnum, "GM_READ_FAIL", "");
		return false;
	}

	ChatPacket(CHAT_TYPE_TALKING, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "몸에서 뭔가 힘이 터져 나오는 기분이야!"));
	ChatPacket(CHAT_TYPE_TALKING, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "뜨거운 무엇이 계속 용솟음치고 있어! 이건, 이것은!"));
	ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "더 높은 경지의 수련을 성공적으로 끝내셨습니다."));
...
bool CHARACTER::LearnSkillByBook(DWORD dwSkillVnum, BYTE bProb)
{
	...
	if (FN_should_check_exp(this))
	{
		...
		if ( GetExp() < need_exp )
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "경험치가 부족하여 책을 읽을 수 없습니다."));
			return false;
		}
	}
	...
	if (pkSk->dwType != 0)
	{
		...
		{
			if (GetSkillMasterType(dwSkillVnum) > SKILL_MASTER)
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "이 스킬은 책으로 더이상 수련할 수 없습니다."));
			else
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "이 스킬은 아직 책으로 수련할 경지에 이르지 않았습니다."));
			return false;
		}
	}
			...
			if (pPC)
			{
				...
				if (number(1, 100) > percent)
				{
					...
					if (read_count >= need_bookcount)
					{
						...
						ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "책으로 더 높은 경지의 수련을 성공적으로 끝내셨습니다."));
					...
					else
					{
						...
						{
							case 1:
								ChatPacket(CHAT_TYPE_TALKING, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "어느정도 이 기술에 대해 이해가 되었지만 조금 부족한듯 한데.."));
								break;

							case 2:
								ChatPacket(CHAT_TYPE_TALKING, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "드디어 끝이 보이는 건가...  이 기술은 이해하기가 너무 힘들어.."));
								break;

							case 3:
							default:
								ChatPacket(CHAT_TYPE_TALKING, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "열심히 하는 배움을 가지는 것만이 기술을 배울수 있는 유일한 길이다.."));
								break;
						}

						ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "%d 권을 더 읽어야 수련을 완료 할 수 있습니다."), need_bookcount - read_count);
						return true;
					}
				}
			}
...
EVENTFUNC(skill_gwihwan_event)
{
	...
	{
		//실패
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "귀환에 실패하였습니다."));
	}
	return 0;
}
...
bool CHARACTER::UseSkill(DWORD dwVnum, LPCHARACTER pkVictim, bool bUseGrandMaster)
{
	...
	if ( IsInOXMap() )
	{
		ChatPacket(CHAT_TYPE_INFO, "[OX-Event] Du kannst hier keine Fertigkeiten aktivieren!");
		return false;
	}
	...
	if (dwVnum == SKILL_HORSE_SUMMON)
	{
		...
		if (GetHorseLevel() <= 0)
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "말이 없습니다. 마굿간 경비병을 찾아가세요."));
		else
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "말 소환 아이템을 사용하세요."));
	...
	else
	{
		...
		if (test_server)
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "%s SP소모: %d"), pkSk->szName, iNeededSP);
...
void CHARACTER::SkillLearnWaitMoreTimeMessage(DWORD ms)
{
	...
	//
	if (ms < 3 * 60)
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "몸 속이 뜨겁군. 하지만 아주 편안해. 이대로 기를 안정시키자."));
	else if (ms < 5 * 60)
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "그래, 천천히. 좀더 천천히, 그러나 막힘 없이 빠르게!"));
	else if (ms < 10 * 60) // 10분
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "그래, 이 느낌이야. 체내에 기가 아주 충만해."));
	else if (ms < 30 * 60) // 30분
	{
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "다 읽었다! 이제 비급에 적혀있는 대로 전신에 기를 돌리기만 하면,"));
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "그것으로 수련은 끝난 거야!"));
	}
	else if (ms < 1 * 3600) // 1시간
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "이제 책의 마지막 장이야! 수련의 끝이 눈에 보이고 있어!"));
	else if (ms < 2 * 3600) // 2시간
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "얼마 안 남았어! 조금만 더!"));
	else if (ms < 3 * 3600)
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "좋았어! 조금만 더 읽으면 끝이다!"));
	else if (ms < 6 * 3600)
	{
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "책장도 이제 얼마 남지 않았군."));
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "뭔가 몸 안에 힘이 생기는 기분인 걸."));
	}
	else if (ms < 12 * 3600)
	{
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "이제 좀 슬슬 가닥이 잡히는 것 같은데."));
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "좋아, 이 기세로 계속 나간다!"));
	}
	else if (ms < 18 * 3600)
	{
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "아니 어떻게 된 게 종일 읽어도 머리에 안 들어오냐."));
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "공부하기 싫어지네."));
	}
	else //if (ms < 2 * 86400)
	{
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "생각만큼 읽기가 쉽지가 않군. 이해도 어렵고 내용도 난해해."));
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "이래서야 공부가 안된다구."));
	}
	/*
	   str = "30%";
	   else if (ms < 3 * 86400)
