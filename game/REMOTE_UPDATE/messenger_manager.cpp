...
void MessengerManager::__AddToBlockList(MessengerManager::keyA account, MessengerManager::keyA companion)
{
	...
	if (d)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "%s bloklandi"), companion.c_str());
	}
...
void MessengerManager::__RemoveFromBlockList(MessengerManager::keyA account, MessengerManager::keyA companion)
{
	...
	if (d)
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "<메신져> %s 님을 메신저에서 삭제하였습니다."), companion.c_str());
}
...
void MessengerManager::RequestToAdd(LPCHARACTER ch, LPCHARACTER target)
{
	...
	{
	    ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "상대방이 친구 추가를 받을 수 없는 상태입니다."));
	    return;
	}
...
void MessengerManager::__AddToList(MessengerManager::keyA account, MessengerManager::keyA companion)
{
	...
	if (d)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "<메신져> %s 님을 친구로 추가하였습니다."), companion.c_str());
	}
