...
namespace quest
{
	...
	// d.set_flag_from_map_index(mapIndex,flagName,flagNumber)
	int dungeon_set_flag_from_map_index(lua_State* L)
	{
		if (!lua_isstring(L,1) || !lua_isnumber(L,2) || !lua_isnumber(L,3))
		{
			sys_err("dungeon_set_flag_from_map_index: wrong get flag:: mapIndex,flagName,flagNumber");
			return 0;
		}
		...
		if (dwMapIndex)
		{
			...
			{
				sys_err("dungeon_set_flag_from_map_index: no dungeon !!!");
				return 0;
			}
		}
		else
...
//						M2_DESTROY_CHARACTER(pChar);
	...
	// Gibt die lokale x Koordinate des UniqueVID zur?k. [NICHT GETESTET]
	int dungeon_get_unique_pos_x(lua_State* L)
	{
		DWORD UniqueVID = (DWORD) lua_tonumber(L, 1);
		LPCHARACTER ch = CHARACTER_MANAGER::instance().Find(UniqueVID);
	...
	// Gibt die lokale y Koordinate des UniqueVID zur?k.
	int dungeon_get_unique_pos_y(lua_State* L)
	{
		DWORD UniqueVID = (DWORD) lua_tonumber(L, 1);
		LPCHARACTER ch = CHARACTER_MANAGER::instance().Find(UniqueVID);
	...
	// L?st das UniqueVID einen Skill ausf?ren. [NICHT GETESTET]
	int dungeon_unique_skill(lua_State* L)
	{
		
		DWORD UniqueVID = (DWORD) lua_tonumber(L, 1);
		LPCHARACTER ch = CHARACTER_MANAGER::instance().Find(UniqueVID);	
		ch->SendMovePacket(FUNC_MOB_SKILL, lua_tonumber(L, 2), ch->GetX(), ch->GetY(), 0, get_dword_time());
		return 0;
	}

	// Erstellt einen Dungeon gibt die Map-Index zuruck. [GETESTET]
	int dungeon_create_and_return_mid(lua_State* L)
	{
		if (lua_gettop(L)<1)
		{
			sys_err("D Create And Return: Fehlende Infos.");
			return 0;
		}
		...
		if (!pDungeon)
		{
			sys_err("Dungeon kann nicht erstellt werden %d", lMapIndex);
			return 0;
		}
		//sys_err("Dungeon wurde erstellt: %d", pDungeon->GetMapIndex());
		lua_pushnumber(L, pDungeon->GetMapIndex());
		return 1;
	}
	
	int dungeon_count_players(lua_State * L)
	{
		CQuestManager& q = CQuestManager::instance();
		LPDUNGEON pDungeon = q.GetCurrentDungeon();

		if (pDungeon)
			lua_pushnumber(L, pDungeon->CountRealPlayer());
		else
		{
			sys_err("not in a dungeon");
			lua_pushnumber(L, 0);
	...
	int dungeon_set_level_stop(lua_State* L)
	{
		CQuestManager & q = CQuestManager::instance();
		...
		f.flagname = string ("trainingshalle.level_stop");
		f.value = (int) rint(lua_tonumber(L, 1));
	...
	void RegisterDungeonFunctionTable()
	{
		luaL_reg dungeon_functions[] =
		{
			...
			{ "get_unique_pos_y",			dungeon_get_unique_pos_y	},
			{ "unique_skill",			dungeon_unique_skill	},
			{ "create_and_return_mid",			dungeon_create_and_return_mid	},
			...
			{ "setf_from_map_index",	dungeon_set_flag_from_map_index	},
			...
			{ "set_level_stop",			dungeon_set_level_stop	},
