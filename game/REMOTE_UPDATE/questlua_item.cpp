...
namespace quest
{
	...
	int item_remove_secure(lua_State* L)
	{
		CQuestManager& q = CQuestManager::instance();
		LPITEM item = q.GetCurrentItem();
		if (item != NULL) {
			if (q.GetCurrentCharacterPtr() == item->GetOwner()) {
				...
				int remove_check = item->GetCount() - count;
				if (remove_check <= 0)
					ITEM_MANAGER::instance().RemoveItem(item);
				else
					item->SetCount(remove_check);
			} else {
				sys_err("Tried to remove invalid item %p", get_pointer(item));
			}
			q.ClearCurrentItem();
...
#define NS_ITEM_GETMODE0(x)	\
	...
	void RegisterITEMFunctionTable()
	{
		...
		luaL_reg item_functions[] =
		{
			...
			{ "remove",		item_remove		},
			{ "remove_secure",		item_remove_secure		},
			...
			{ "set_attr",			item_set_attr},	// [return nothing]
