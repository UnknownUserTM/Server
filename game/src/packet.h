...
//	HEADER_CG_RUNUP_MATRIX_ANSWER	= 201,
	...
	// END_OF_SUPPORT_BGM
	HEADER_GC_ITEM_PICKUP				= 139,
	HEADER_GC_SHOUT_FILTER_LIST			= 140,
	...
	// QuestDialog
	HEADER_GC_QUEST_DIALOG_TITLE			= 220,
	HEADER_GC_QUEST_DIALOG_DESC				= 221,
	HEADER_GC_QUEST_DIALOG_TARGET			= 222,
	HEADER_GC_QUEST_DIALOG_REWARD			= 223,
...
typedef struct SPacketGGAnnouncement
{
	BYTE	bHeader;
	int		iType;
	int		iMapIndex;
	char	szText[CHAT_MAX_LEN + 1];
	char	szLanguage[LANGUAGE_MAX_LEN + 1];
} TPacketGGAnnouncement;

typedef struct SPacketCGShoutFilter
{
	BYTE 	header;
	int		bSubHeader;
	int		iType;
...
} TPacketCGShoutFilter;

typedef struct SPacketGCLoadShoutFilterList
{
	BYTE 	header;
	BYTE	bFilterList[LANGUAGE_MAX_NUM + 1];
} TPacketGCLoadShoutFilterList;

typedef struct SPacketGCItemPickup
{
	BYTE 	header;
...
typedef struct command_messenger_request_warp_by_name
{
	BYTE length;
} TPacketCGMessengerRequestWarpByName;

typedef struct command_messenger_summon_by_name
{
	BYTE length;
} TPacketCGMessengerSummonByName;
...
typedef struct packet_questdialog_title_paket
{
	BYTE byHeader;
	int	title;
} TPacketGCQuestDialogTitle;

typedef struct packet_questdialog_desc_paket
{
	BYTE byHeader;
	int	desc;
} TPacketGCQuestDialogDesc;

typedef struct packet_questdialog_target_paket
{
	BYTE byHeader;
	int		type;
	int		vnum;
	int		count_min;
	int		count_max;
} TPacketGCQuestDialogTarget;

typedef struct packet_questdialog_reward_paket
{
	BYTE byHeader;
	int		type;
	int		vnum;
	int		count;
} TPacketGCQuestDialogReward;
#pragma pack()
#endif