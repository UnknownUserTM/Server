...
bool CHARACTER::ExchangeStart(LPCHARACTER victim)
{
	...
	{
		//ben blokladım hacı
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "%s blokkk"), victim->GetName());
		return false;
	}
	if (MessengerManager::instance().IsBlocked_Target(GetName(), victim->GetName()))
	{
		//o blokladı hacı
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "%s blokkk_me"), victim->GetName());
		return false;
	}
	#endif
	...
	if (IsObserverMode())
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "관전 상태에서는 교환을 할 수 없습니다."));
		return false;
	}
	...
	{
		ChatPacket( CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "다른 거래창이 열려있을경우 거래를 할수 없습니다." ) );
		return false;
	}

	if ( victim->IsOpenSafebox() || victim->GetShopOwner() || victim->GetMyShop() || victim->IsCubeOpen() )
	{
		ChatPacket( CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "상대방이 다른 거래중이라 거래를 할수 없습니다." ) );
		return false;
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "상대방이 교환 거부 상태입니다."));
		return false;
	}
...
bool CExchange::AddItem(TItemPos item_pos, BYTE display_pos)
{
	...
	{
		m_pOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(m_pOwner->GetLanguage(), "아이템을 건네줄 수 없습니다."));
		return false;
	}
...
bool CExchange::Accept(bool bAccept)
{
	...
	if (m_bAccept && GetCompany()->m_bAccept)
	{
		...
		if (!Check(&iItemCount))
		{
			GetOwner()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetOwner()->GetLanguage(), "돈이 부족하거나 아이템이 제자리에 없습니다."));
			victim->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(victim->GetLanguage(), "상대방의 돈이 부족하거나 아이템이 제자리에 없습니다."));
			goto EXCHANGE_END;
		}
		...
		if (!CheckSpace())
		{
			GetOwner()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetOwner()->GetLanguage(), "상대방의 소지품에 빈 공간이 없습니다."));
			victim->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(victim->GetLanguage(), "소지품에 빈 공간이 없습니다."));
			goto EXCHANGE_END;
		}
		...
		{
			victim->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(victim->GetLanguage(), "돈이 부족하거나 아이템이 제자리에 없습니다."));
			GetOwner()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetOwner()->GetLanguage(), "상대방의 돈이 부족하거나 아이템이 제자리에 없습니다."));
			goto EXCHANGE_END;
		}

		if (!GetCompany()->CheckSpace())
		{
			victim->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(victim->GetLanguage(), "상대방의 소지품에 빈 공간이 없습니다."));
			GetOwner()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetOwner()->GetLanguage(), "소지품에 빈 공간이 없습니다."));
			goto EXCHANGE_END;
		}
		...
		if (Done())
		{
			...
			if (GetCompany()->Done())
			{
				...
				GetOwner()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetOwner()->GetLanguage(), "%s 님과의 교환이 성사 되었습니다."), victim->GetName());
				victim->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(victim->GetLanguage(), "%s 님과의 교환이 성사 되었습니다."), GetOwner()->GetName());
