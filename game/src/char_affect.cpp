...
void CHARACTER::ClearAffect(bool bSave)
{
	...
	while (it != m_list_pkAffect.end())
	{
		...
		if (bSave)
		{
			...
			DWORD pkAff_blend[6] = {AFFECT_POTION_1, AFFECT_POTION_2, AFFECT_POTION_3, AFFECT_POTION_4, AFFECT_POTION_5, AFFECT_POTION_6};
...
bool CHARACTER::AddAffect(DWORD dwType, BYTE bApplyOn, long lApplyValue, DWORD dwFlag, long lDuration, long lSPCost, bool bOverride, bool IsCube )
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "운영자 제제로 채팅이 금지 되었습니다."));
	}
	// END_OF_CHAT_BLOCK
	...
	switch (dwType)
	{
...
#endif
		{
			...
			{
				ChatPacket(CHAT_TYPE_INFO, "<AddAffect> hat die Fahigkeit zum Empfangen blockiert(%s) weil die Macht (%ld%%) kleiner ist als aktuell. (%ld%%).", CSkillManager::instance().Get(dwType)->szName, lApplyValue, pkAffect->lApplyValue);
				return false;
			}
		}
		break;
...
bool CHARACTER::RemoveAffect(DWORD dwType)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "채팅 금지가 풀렸습니다."));
	}
	// END_OF_CHAT_BLOCK
