...
void Acce_clean_item(LPCHARACTER ch)
{
	...
	for (int i=0; i<ACCE_MAX_NUM; ++i)
	{
		...
		if (acce_item[i]->GetSubType() == COSTUME_ACCE)
...
void Acce_open(LPCHARACTER ch)
{
	...
	if (ch->IsAcceOpen())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "The Acce creation window is already opened."));
		return;
	}
	
	if ( ch->GetExchange() || ch->GetMyShop() || ch->GetShopOwner() || ch->IsOpenSafebox() || ch->IsAcceOpen() )
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "다른 거래중(창고,교환,상점)에는 사용할 수 없습니다."));
		return;
	}
...
bool Acce_make(LPCHARACTER ch)
{
	...
	if (!(ch)->IsAcceOpen())
	{
		(ch)->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE((ch)->GetLanguage(), "The Acce creation window is not open."));
		return false;
	}
	...
	if (items[0] == NULL || items[1] == NULL || items[0]->GetVnum() != items[1]->GetVnum())
	{
		(ch)->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE((ch)->GetLanguage(), "Drag the items into the window."));
		return false;
	}
	...
	if (NULL == acce_proto)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "제조 재료가 부족합니다"));
		return false;
	}
	
	if (ch->GetGold() < acce_proto->gold)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "돈이 부족하거나 아이템이 제자리에 없습니다."));
		return false;
	}
	...
	if (percent_number <= acce_proto->percent)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "Success!"));
		ch->ChatPacket(CHAT_TYPE_COMMAND, "acce success %d %d", reward_value->vnum, reward_value->count);
		...
		new_item = ch->AutoGiveAcce(reward_value->vnum, reward_value->count, abs_chance);
		LogManager::instance().AcceLog(ch->GetPlayerID(), ch->GetX(), ch->GetY(), reward_value->vnum, new_item->GetID(), reward_value->count, abs_chance, 1);
		return true;
	}
	else
	{
		acce_proto->remove_material(ch, 0);
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "Failed!"));
		ch->ChatPacket(CHAT_TYPE_COMMAND, "Acce fail");
		LogManager::instance().AcceLog(ch->GetPlayerID(), ch->GetX(), ch->GetY(), reward_value->vnum, 0, 0, 0, 0);
		return false;
	}
...
bool Acce_absorption_make(LPCHARACTER ch)
{
	...
	{
		(ch)->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE((ch)->GetLanguage(), "You can use a sash to absorb the bonus just from weapons or armours."));
		return false;
	}
	else if (items[0]->GetAcceAttributeCount() > 0)
	{
		(ch)->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE((ch)->GetLanguage(), "You must remove the bonus from sash before continue."));
		return false;
	}
	...
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "The absorption process had success!"));
