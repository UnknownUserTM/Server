...
bool	g_bEmotionSameGender = false;
DWORD	g_bItemCountLimit = 200;
...
char		g_szPublicIP[16] = "149.202.241.224";
#else
...
int g_server_id = 1;
string g_strWebMallURL = "http://unknownusertm.infinity-hosting.com/ishop";
...
static bool __LoadGeneralConfigFile(const char* configName)
{
	...
	while (fgets(buf, 256, fp))
	{
		...
		TOKEN("emotion_same_gender")
		{
			...
			str_to_number(flag, value_string);
			g_bEmotionSameGender = !!flag;

			fprintf(stdout, "EMOTION_SAME_GENDER: %s\n", g_bEmotionSameGender ? "ENABLED" : "DISABLED");
			continue;
		}
		...
		TOKEN("attr_add_chance")
		{
			str_to_number(g_iAttrAddChance, value_string);
			g_iAttrAddChance = MINMAX(1, g_iAttrAddChance, 100);

			fprintf(stdout, "ATTR_ADD_CHANCE: %d\n", g_iAttrAddChance);
			continue;
		}
		...
		TOKEN("base_movement_speed")
		{
			str_to_number(g_iBaseMovementSpeed, value_string);
			g_iBaseMovementSpeed = MINMAX(1, g_iBaseMovementSpeed, 200);

			fprintf(stdout, "BASE_MOVEMENT_SPEED: %d\n", g_iBaseMovementSpeed);
		}
		...
		TOKEN("box_use_limit_time")
		{
			...
			fprintf(stdout, "BOX_USE_LIMIT_TIME: %d\n", g_BuySellTimeLimitValue);
		}
		...
		TOKEN("buysell_limit_time")
		{
			...
			fprintf(stdout, "BUYSELL_LIMIT_TIME: %d\n", g_BuySellTimeLimitValue);
		}
		
		TOKEN("allow_mount_at_guild_war")
		{
			int flag = 0;
			str_to_number(flag, value_string);
			g_NoMountAtGuildWar = !flag;
			
			fprintf(stdout, "ALLOW_MOUNT_AT_GUILD_WAR: %s\n", (!g_NoMountAtGuildWar) ? "DISABLED" : "ENABLED");
		}
		...
		TOKEN("no_mount_at_guild_war")
		{
			...
			fprintf(stdout, "NO_MOUNT_AT_GUILD_WAR: %s\n", (g_NoMountAtGuildWar) ? "ENABLED" : "DISABLED");
		}
		
		TOKEN("metin_stone_drop")
		{
			int flag = 0;
			str_to_number(flag, value_string);
			g_NoDropMetinStone = !!flag;
			
			fprintf(stdout, "METIN_STONE_DROP: %s\n", (g_NoDropMetinStone) ? "DISABLED" : "ENABLED");
		}
...
void CheckClientVersion()
{
	...
	while (it != set.end())
	{
		...
		{
			d->GetCharacter()->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT_CONVERT_LANGUAGE(d->GetCharacter()->GetLanguage(), "클라이언트 버전이 틀려 로그아웃 됩니다. 정상적으로 패치 후 접속하세요."));
