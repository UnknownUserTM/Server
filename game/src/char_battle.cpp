...
void CHARACTER::DeathPenalty(BYTE bTown)
{
	...
	if (GetLevel() < 10)
	{
		...
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "용신의 가호로 경험치가 떨어지지 않았습니다."));
		return;
	}
...
void CHARACTER::Dead(LPCHARACTER pkKiller, bool bImmediateDead)
{
	...
	if (pkKiller && pkKiller->IsPC())
	{
		...
		if(!IsPC())
		{
			if(GetRaceNum() >= 6650 && GetRaceNum() <= 6652)
			{
				int plague_count = quest::CQuestManager::instance().GetEventFlag("plague_count") - 1;
				if(plague_count<=0)
				{
					quest::CQuestManager::instance().SetEventFlag("event_succeed",1);
					SendNoticeMap("Alle Plagentrager wurden vernichtet und die Seuche eingedammt!", GetMapIndex(), true);

				}
				else
				{
					SendNoticeMap("Ein Plagentrager wurde vernichtet! Zerstort auchd en rest um die Plage einzudammen!", GetMapIndex(), true);
					quest::CQuestManager::instance().SetEventFlag("plague_count", plague_count);	
	...
	if (pkKiller && IsPC())
	{
		...
		else
		{
			...
			{
				if (!isAgreedPVP && !isUnderGuildWar && !IsKillerMode() && GetAlignment() >= 0 && !isDuel && !isForked && GetMapIndex() != 361 && IsInBattlezone() != true)
				{
		...
		if (!IS_SET(m_pointsInstant.instant_flag, INSTANT_FLAG_NO_REWARD))
		{
			...
			else
			{
				if (pkKiller->m_dwUnderGuildWarInfoMessageTime < get_dword_time())
				{
					...
					pkKiller->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pkKiller->GetLanguage(), "<길드> 길드전중에는 사냥에 따른 이익이 없습니다."));
				}
			}
		}
	}
	...
	// if (pkKiller->mount_summon)
	// {
		// // pkKiller->ChatPacket(CHAT_TYPE_INFO, "[DEBUG] MountSystem KILL");
		// int moblevelcalc = pkKiller->GetLevel() - GetLevel();
		// if (moblevelcalc < 10 && pkKiller->mount_level < 100) 
		// {
			// // pkKiller->ChatPacket(CHAT_TYPE_INFO, "[DEBUG] MountSystem givePoints");
			// pkKiller->mountsystem_exp();
			// CreateFly(FLY_SP_MEDIUM, pkKiller);
		// }	
...
void CHARACTER::SendDamagePacket(LPCHARACTER pAttacker, int Damage, BYTE DamageFlag)
{
	if (IsPC() == true || (pAttacker->IsPC() == true && pAttacker->GetTarget() == this))
	{
		if(pAttacker->IsPC())
		{
			if(GetRaceNum() >= 6601 && GetRaceNum() <= 6652)
				Damage = 1;
		}
...
bool CHARACTER::Damage(LPCHARACTER pAttacker, int dam, EDamageType type) // returns true if dead
{
	...
	if(IsStone())
	{
		if(IsInFarmMap())
		{
			if(auto_purge_time == 0)
			{
				auto_purge_time = get_global_time() + 900;
				pAttacker->ChatPacket(CHAT_TYPE_INFO,"|cff00ff00[System] Der Metin wird in 15 Min. verschwinden!");
			}
		}
	}
		...
		if (pAttacker)
		{
			...
			if (iPenetratePct)
			{
				...
				if (number(1, 100) <= iPenetratePct)
				{
					...
					if (test_server)
						ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "관통 추가 데미지 %d"), GetPoint(POINT_DEF_GRADE) * (100 + GetPoint(POINT_DEF_BONUS)) / 100);
	...
	else if (type == DAMAGE_TYPE_NORMAL || type == DAMAGE_TYPE_NORMAL_RANGE)
	{
		if (type == DAMAGE_TYPE_NORMAL)
		{
			...
			if (GetPoint(POINT_BLOCK) && number(1, 100) <= GetPoint(POINT_BLOCK))
			{
				if (test_server)
				{
					pAttacker->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pAttacker->GetLanguage(), "%s 블럭! (%d%%)"), GetName(), GetPoint(POINT_BLOCK));
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "%s 블럭! (%d%%)"), GetName(), GetPoint(POINT_BLOCK));
				}
		...
		else if (type == DAMAGE_TYPE_NORMAL_RANGE)
		{
			...
			if (GetPoint(POINT_DODGE) && number(1, 100) <= GetPoint(POINT_DODGE))
			{
				if (test_server)
				{
					pAttacker->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pAttacker->GetLanguage(), "%s 회피! (%d%%)"), GetName(), GetPoint(POINT_DODGE));
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "%s 회피! (%d%%)"), GetName(), GetPoint(POINT_DODGE));
				}
	...
	if (pAttacker)
	{
		...
		// if(GetRaceNum() == 6708)
		// {
			// int hpPerc = (100.f*GetHP())/GetMaxHP();
			// int resistPerc = 80 - hpPerc;
			// dam = 
		...
		if (pAttacker)
		{
			if(pAttacker->GetRaceNum() < 6601 || pAttacker->GetRaceNum() > 6652)
			{
		...
		if (m_bDetailLog)
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "%s[%d]가 공격 위치: %d %d"), pAttacker->GetName(), (DWORD) pAttacker->GetVID(), pAttacker->GetX(), pAttacker->GetY());
		}
	...
	if(pAttacker)
	{
		if (pAttacker->IsMonster())
		{
			if(pAttacker->GetRaceNum() >= 6601 && pAttacker->GetRaceNum() <= 6652)
			{
				BYTE damageFlag = 0;
				damageFlag = DAMAGE_NORMAL;
				int calcDamageScal = (GetMaxHP()*number(2,6)) / 100;
				dam = calcDamageScal;
				SendDamagePacket(pAttacker, dam, damageFlag);
			}			
		}
	...
	{
		if(GetRaceNum() >= 6601 && GetRaceNum() <= 6652)
			//quest::CQuestManager::instance().hit(pAttacker->GetPlayerID());
			dam = 1;
...
static void GiveExp(LPCHARACTER from, LPCHARACTER to, int iExp)
{
	...
	// if (to->mount_summon)
	// {
		// int moblevelcalc = to->GetLevel() - from->GetLevel();
		// if (moblevelcalc < 10 && to->mount_level < 100) 
		// {
			// to->mountsystem_exp();
			// from->CreateFly(FLY_SP_MEDIUM, to);
		// }		
	// }
	// marriage
	{
...
namespace NPartyExpDistribute
{
	...
	struct FPartyDistributor
	{
		...
		FPartyDistributor(LPCHARACTER center, int member_count, int total, DWORD iExp, int iMode, int iMapIndex)
			: total(total), c(center), x(center->GetX()), y(center->GetY()), _iExp(iExp), m_iMode(iMode), m_iMemberCount(member_count), m_iMapIndex(iMapIndex)
			{
				if (m_iMemberCount == 0)
					m_iMemberCount = 1;
			};
		...
		void operator () (LPCHARACTER ch)
		{
			if (DISTANCE_APPROX(ch->GetX() - x, ch->GetY() - y) <= PARTY_DEFAULT_RANGE && ch->GetMapIndex() == m_iMapIndex)
			{
				DWORD iExp2 = 0;
...
typedef struct SDamageInfo
{
	...
	{		
		// if (pAttacker->mount_summon)
		// {
			// // pkKiller->ChatPacket(CHAT_TYPE_INFO, "[DEBUG] MountSystem KILL");
			// int moblevelcalc = pAttacker->GetLevel() - ch->GetLevel();
			// if (moblevelcalc < 10 && pAttacker->mount_level < 100) 
			// {
				// // pkKiller->ChatPacket(CHAT_TYPE_INFO, "[DEBUG] MountSystem givePoints");
				// pAttacker->mountsystem_exp();
				// ch->CreateFly(FLY_SP_MEDIUM, pAttacker);
			// }	
				// // pkKiller->ChatPacket(CHAT_TYPE_INFO, "[DEBUG] MountSystem KILL end");
		// }
		if (pAttacker)
		...
		else if (pParty)
		{
			...
			if (pParty->GetExpCentralizeCharacter())
			{
				...
				if (DISTANCE_APPROX(ch->GetX() - tch->GetX(), ch->GetY() - tch->GetY()) <= PARTY_DEFAULT_RANGE && ch->GetMapIndex() == tch->GetMapIndex())
				{
			...
			NPartyExpDistribute::FPartyDistributor fDist(ch, f.member_count, f.total, iExp, pParty->GetExpDistributionMode(), ch->GetMapIndex());
