...
///Add
	...
	int pc_skillbook_simulate(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		lua_pushboolean(L,ch->LearnSkillByBook(lua_tonumber(L, 1)));
		return 1;
	}
	
	int pc_is_in_battlezone(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		lua_pushboolean(L,ch->GetMapIndex()==361);
		return 1;		
	}
	
	int pc_request_battlezone_enter(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		ch->ChatPacket(CHAT_TYPE_COMMAND,"battlezone enter_request");
		return 0;		
	}	
	
	int pc_update_enter_battlezone(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		ch->ChatPacket(CHAT_TYPE_COMMAND,"battlezonepoints %d",ch->GetQuestFlag("battlezone.points"));
		return 0;			
	}

	int pc_battlezone_kill(lua_State* L)
	{
		DWORD pid = (DWORD) lua_tonumber(L, 1);
		LPCHARACTER pkKiller = CQuestManager::instance().GetCurrentCharacterPtr();
		LPCHARACTER ch = CHARACTER_MANAGER::instance().FindByPID(pid);
		
		//pkKiller->ChatPacket(CHAT_TYPE_NOTICE,"Test");
		//ch->ChatPacket(CHAT_TYPE_NOTICE,"Test");
		
		int pkKillerPoints 			= pkKiller->GetQuestFlag("battlezone.points");
		int pkKillerKills 			= pkKiller->GetQuestFlag("battlezone.battlezone_kills");
		int pkKillerPointsComplete 	= pkKiller->GetQuestFlag("battlezone.battlezone_points_complete");

		int chPoints       			= ch->GetQuestFlag("battlezone.points");
		int chDeads					= ch->GetQuestFlag("battlezone.battlezone_dead");
		int chPointsLost 			= ch->GetQuestFlag("battlezone.battlezone_points_lost");
		int	chRespawnTime			= ch->GetQuestFlag("battlezone.respawn_time");
		bool chIsExit				= ch->GetQuestFlag("battlezone.leave_time") > get_global_time();
		
		
		if(chIsExit)
		{
			// Point calc
			int chHalfPoints = (chPoints*50) / 100;
			int chMaxHaltPoints = MAX(1,chHalfPoints);
			
			// QUestFlagPoints & ClientUpdate
			pkKiller->SetQuestFlag("battlezone.points", pkKillerPoints+chMaxHaltPoints);
			pkKiller->ChatPacket(CHAT_TYPE_COMMAND,"battlezonepoints %d", pkKiller->GetQuestFlag("battlezone.points"));
				
			// QUestFlagPoints & ClientUpdate
			ch->SetQuestFlag("battlezone.points", chPoints-chMaxHaltPoints);
			ch->ChatPacket(CHAT_TYPE_COMMAND,"battlezonepoints %d", ch->GetQuestFlag("battlezone.points"));
				
				
			// Exit Event break
			ch->battlezone_BreakExit();
				
			// Chat
			ch->ChatPacket(CHAT_TYPE_NOTICE,"Dein Fluchtversuch wurde unterbrochen. Du verlierst %d Punkt(e).", chMaxHaltPoints);
			pkKiller->ChatPacket(CHAT_TYPE_NOTICE,"Du hast %d Punkt(e) erhalten. Deine Gesamtpunktzahl ist %d.", chMaxHaltPoints, pkKiller->GetQuestFlag("battlezone.points"));				
			
			
			// Statistiken
			pkKiller->SetQuestFlag("battlezone.battlezone_kills", pkKillerKills+1);
			pkKiller->SetQuestFlag("battlezone.battlezone_points_complete", pkKillerPointsComplete+chMaxHaltPoints);	
				
			ch->SetQuestFlag("battlezone.battlezone_points_lost", chPointsLost+chMaxHaltPoints);
			ch->SetQuestFlag("battlezone.battlezone_dead", chDeads+1);
		}
		else
		{
				
			
			pkKiller->SetQuestFlag("battlezone.points", pkKillerPoints+1);
			pkKiller->SetQuestFlag("battlezone.battlezone_kills", pkKillerKills+1);
			pkKiller->SetQuestFlag("battlezone.battlezone_points_complete", pkKillerPointsComplete+1);
			pkKiller->ChatPacket(CHAT_TYPE_NOTICE,"Du hast einen Punkt erhalten. Deine Gesamtpunktzahl ist %d.", pkKiller->GetQuestFlag("battlezone.points"));
			pkKiller->ChatPacket(CHAT_TYPE_COMMAND,"battlezonepoints %d", pkKiller->GetQuestFlag("battlezone.points"));
			
			if (chPoints > 0)
			{
				ch->SetQuestFlag("battlezone.points", chPoints-1);
				ch->ChatPacket(CHAT_TYPE_NOTICE,"Du hast einen Punkt verloren. Deine Gesamtpunktzahl ist %d.", ch->GetQuestFlag("battlezone.points"));
				ch->ChatPacket(CHAT_TYPE_COMMAND,"battlezonepoints %d.", ch->GetQuestFlag("battlezone.points"));
				ch->SetQuestFlag("battlezone.battlezone_points_lost", chPointsLost+1);
			}
			if (chRespawnTime > 0)
				ch->SetQuestFlag("battlezone.respawn_time", chRespawnTime-10);
			ch->SetQuestFlag("battlezone.battlezone_dead", chDeads+1);
		}
	}
	
	int pc_battlezone_succeed(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		//ch->EffectPacket(SE_SUCCESS);
		ch->ChatPacket(CHAT_TYPE_NOTICE,"Du hast konntest erfolgreich %d Punkt(e) aus der Kampfzone bergen!", ch->GetQuestFlag("battlezone.success"));
		
		//cheerup
		ch->ChatPacket(CHAT_TYPE_COMMAND,"battlzezone succeed");
		ch->ChatPacket(CHAT_TYPE_COMMAND,"buffitem effect_item_buff_battlezone");
		
		
		ch->SetQuestFlag("battlezone.success",0);
	}
	
	int pc_is_new_mountsystem_summon(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		lua_pushboolean(L,ch->mount_summon);
		return 1;		
	}
	...
	int pc_warp_local(lua_State * L)
	{
		...
		{
			pkChr->ChatPacket( CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pkChr->GetLanguage(), "거래창,창고 등을 연 상태에서는 다른곳으로 이동할수 없습니다" ));
		...
		{
			pkChr->ChatPacket( CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pkChr->GetLanguage(), "거래 후 1분 이내에는 다른지역으로 이동 할 수 없습니다." ) );
			return;
		}
	...
	int pc_give_gold(lua_State* L)
	{
		...
		int64_t iAmount = lua_tonumber(L, 1);
	...
	int pc_give_item_from_special_item_group(lua_State* L)
	{
		...
		for (int i = 0; i < count; i++)
		{
			if (!item_gets[i])
			{
				if (dwVnums[i] == 1)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "돈 %d 냥을 획득했습니다."), dwCounts[i]);
				}
				else if (dwVnums[i] == 2)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "나무에서 부터 신비한 빛이 나옵니다."));
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "%d의 경험치를 획득했습니다."), dwCounts[i]);
				}
			}
		}
		return 0;
...
// #endif
	...
	int pc_mount_level(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr(); // Spieler
		// LPCHARACTER tch = CHARACTER_MANAGER::instance().Find((DWORD)lua_tonumber(L, 1)); // Monster
		
		if (ch->mount_summon)
		{
			// pkKiller->ChatPacket(CHAT_TYPE_INFO, "[DEBUG] MountSystem KILL");
			// int moblevelcalc = ch->GetLevel() - tch->GetLevel();
			// if (moblevelcalc < 10 && ch->mount_level < 100) 
			// {
				// pkKiller->ChatPacket(CHAT_TYPE_INFO, "[DEBUG] MountSystem givePoints");
			ch->mountsystem_exp();
				// tch->CreateFly(FLY_SP_MEDIUM, ch);
			// }	
				// pkKiller->ChatPacket(CHAT_TYPE_INFO, "[DEBUG] MountSystem KILL end");
		}
		return 0;
	}

	int pc_set_mount_pet_costume_cd(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr(); // Spieler
		// LPCHARACTER tch = CHARACTER_MANAGER::instance().Find((DWORD)lua_tonumber(L, 1)); // Monster
		ch->last_mount_pet_costume_use = get_global_time() + 3;
		return 0;
	}
	
	int pc_get_mount_pet_costume_cd(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr(); // Spieler
		if(ch->last_mount_pet_costume_use < get_global_time())
		{
			lua_pushboolean(L, false);
			return 1;
		}
	...
	int pc_open_shop0(lua_State* L)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "다른 거래창이 열린상태에서는 상점거래를 할수 가 없습니다."));
			return 0;
		}
	...
	int pc_send_quest_intro_desc(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
	...
	int pc_give_biologist_bonus_reward(lua_State* L)
	{
		...
		switch (questIndex)
		{
			...
			case 2: // 20% Doppeldrop
				ch->AddAffect(AFFECT_BIOLOGIST_BONUS_2, POINT_ITEM_DROP_BONUS, 20, AFF_NONE, INFINITE_AFFECT_DURATION, 0, true, true);
				
			
			case 3: // 5 Monster, 5% Boss
				ch->AddAffect(AFFECT_BIOLOGIST_BONUS_3_1, POINT_ATTBONUS_MONSTER, 5, AFF_NONE, INFINITE_AFFECT_DURATION, 0, true, true);
				ch->AddAffect(AFFECT_BIOLOGIST_BONUS_3_2, POINT_ATTBONUS_BOSS, 5, AFF_NONE, INFINITE_AFFECT_DURATION, 0, true, true);
				return 0;	
				
			case 4: // 50 Angriffswert
				ch->AddAffect(AFFECT_BIOLOGIST_BONUS_4, POINT_ATT_BONUS, 50, AFF_NONE, INFINITE_AFFECT_DURATION, 0, true, true);
				return 0;
				
			case 5: // 5 Metin
				ch->AddAffect(AFFECT_BIOLOGIST_BONUS_5, POINT_ATTBONUS_STONE, 5, AFF_NONE, INFINITE_AFFECT_DURATION, 0, true, true);
				return 0;	
				
			case 6: // 10 Krit
				ch->AddAffect(AFFECT_BIOLOGIST_BONUS_6, POINT_CRITICAL_PCT, 10, AFF_NONE, INFINITE_AFFECT_DURATION, 0, true, true);
				return 0;	
			...
			case 8: // 5 Monsterwiderstand
				ch->AddAffect(AFFECT_BIOLOGIST_BONUS_8, POINT_DEFBONUS_MONSTER, 5, AFF_NONE, INFINITE_AFFECT_DURATION, 0, true, true);
				return 0;
				
			case 9: // 10 Monster, 5 Monsterwiderstand
				ch->AddAffect(AFFECT_BIOLOGIST_BONUS_9_1, POINT_ATTBONUS_MONSTER, 10, AFF_NONE, INFINITE_AFFECT_DURATION, 0, true, true);
				ch->AddAffect(AFFECT_BIOLOGIST_BONUS_9_2, POINT_DEFBONUS_MONSTER, 5, AFF_NONE, INFINITE_AFFECT_DURATION, 0, true, true);
				return 0;				
				
			case 10: // 10 Metins, 10 Boss
				ch->AddAffect(AFFECT_BIOLOGIST_BONUS_10_1, POINT_ATTBONUS_STONE, 10, AFF_NONE, INFINITE_AFFECT_DURATION, 0, true, true);
				ch->AddAffect(AFFECT_BIOLOGIST_BONUS_10_2, POINT_ATTBONUS_BOSS, 10, AFF_NONE, INFINITE_AFFECT_DURATION, 0, true, true);
				return 0;				
			
			case 11: // 10 PlayerDEF, 4% Stark gegen Spieler
				ch->AddAffect(AFFECT_BIOLOGIST_BONUS_11_1, POINT_DEFBONUS_PLAYER, 10, AFF_NONE, INFINITE_AFFECT_DURATION, 0, true, true);
				ch->AddAffect(AFFECT_BIOLOGIST_BONUS_11_2, POINT_ATTBONUS_PLAYER, 4, AFF_NONE, INFINITE_AFFECT_DURATION, 0, true, true);
				return 0;				
			
			case 12: // 10 Kritdeff, 10 DB Def
				ch->AddAffect(AFFECT_BIOLOGIST_BONUS_12_1, POINT_RESIST_CRITICAL, 10, AFF_NONE, INFINITE_AFFECT_DURATION, 0, true, true);
				ch->AddAffect(AFFECT_BIOLOGIST_BONUS_12_2, POINT_RESIST_PENETRATE, 10, AFF_NONE, INFINITE_AFFECT_DURATION, 0, true, true);
				return 0;				
	...
	int pc_remove_all_biologist_bonus(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();

		ch->RemoveAffect(AFFECT_BIOLOGIST_BONUS_1);
				
		ch->RemoveAffect(AFFECT_BIOLOGIST_BONUS_2);
				
			
		ch->RemoveAffect(AFFECT_BIOLOGIST_BONUS_3_1);
		ch->RemoveAffect(AFFECT_BIOLOGIST_BONUS_3_2);
				
		ch->RemoveAffect(AFFECT_BIOLOGIST_BONUS_4);
				
		ch->RemoveAffect(AFFECT_BIOLOGIST_BONUS_5);
				
		ch->RemoveAffect(AFFECT_BIOLOGIST_BONUS_6);
				
		ch->RemoveAffect(AFFECT_BIOLOGIST_BONUS_7); 
				
		ch->RemoveAffect(AFFECT_BIOLOGIST_BONUS_8);
				
		ch->RemoveAffect(AFFECT_BIOLOGIST_BONUS_9_1); 
		ch->RemoveAffect(AFFECT_BIOLOGIST_BONUS_9_2);
				
		ch->RemoveAffect(AFFECT_BIOLOGIST_BONUS_10_1);
		ch->RemoveAffect(AFFECT_BIOLOGIST_BONUS_10_2);
			
		ch->RemoveAffect(AFFECT_BIOLOGIST_BONUS_11_1);
		ch->RemoveAffect(AFFECT_BIOLOGIST_BONUS_11_2);
			
		ch->RemoveAffect(AFFECT_BIOLOGIST_BONUS_12_1);
		ch->RemoveAffect(AFFECT_BIOLOGIST_BONUS_12_2); 
	...
	void RegisterPCFunctionTable()
	{
		luaL_reg pc_functions[] =
		{
			...
			{ "skillbook_simulate",	pc_skillbook_simulate	},
			{ "is_new_mountsystem_summon",	pc_is_new_mountsystem_summon	},
			{ "is_battlezone",	pc_is_in_battlezone	},
			{ "request_battlezone_enter",	pc_request_battlezone_enter	},
			{ "update_enter_battlezone",	pc_update_enter_battlezone	},
			{ "battlezone_kill",	pc_battlezone_kill	},
			{ "battlezone_succeed",	pc_battlezone_succeed	},
			{ "mount_level",	pc_mount_level	},
			{ "set_mount_pet_costume_cd",	pc_set_mount_pet_costume_cd	},
			...
			{ "remove_all_biologist_bonus",	pc_remove_all_biologist_bonus	},		
