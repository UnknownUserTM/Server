...
int battle_melee_attack(LPCHARACTER ch, LPCHARACTER victim)
{
	...
	if (timed_event_cancel(ch))
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "전투가 시작 되어 취소 되었습니다."));

	if (timed_event_cancel(victim))
		victim->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(victim->GetLanguage(), "전투가 시작 되어 취소 되었습니다."));
