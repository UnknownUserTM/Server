...
int CItem::FindEquipCell(LPCHARACTER ch, int iCandidateCell)
{
	...
	if ((0 == GetWearFlag() || ITEM_TOTEM == GetType()) && ITEM_COSTUME != GetType() && ITEM_DS != GetType() && ITEM_SPECIAL_DS != GetType() && ITEM_RING != GetType() && ITEM_BELT != GetType() && ITEM_COSTUME_WEAPON != GetType())
		return -1;
	...
	else if (GetType() == ITEM_COSTUME_WEAPON)
	{
...
void CItem::ModifyPoints(bool bAdd)
{
	...
	{
		case ITEM_WEAPON:
			if ( m_pOwner->GetWear(WEAR_COSTUME_WEAPON) != 0 && m_pOwner->GetQuestFlag("costweap.change") != 1 )
				break;
		case ITEM_PICK:
		case ITEM_ROD:
			{
		...
		case ITEM_COSTUME_WEAPON:
			{
				DWORD toSetValue = this->GetVnum();
				EParts toSetPart = PART_WEAPON;
				quest::PC* pPC = quest::CQuestManager::instance().GetPC(m_pOwner->GetPlayerID());
				
				if (!bAdd)
				{
					const CItem* weapon = m_pOwner->GetWear(WEAR_WEAPON);
					toSetValue = (NULL != weapon) ? weapon->GetVnum() : m_pOwner->GetOriginalPart(PART_WEAPON);						
				}
				
				if (pPC)
				{
					int costchange = pPC->GetFlag("costweap.change");
					if (costchange > 0)
					{
						const CItem* weapon = m_pOwner->GetWear(WEAR_WEAPON);
						toSetValue = (NULL != weapon) ? weapon->GetVnum() : m_pOwner->GetOriginalPart(PART_WEAPON);						
...
void CItem::SetAccessorySocketDownGradeTime(DWORD time)
{
	...
	if (test_server && GetOwner())
		GetOwner()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetOwner()->GetLanguage(), "%s에서 소켓 빠질때까지 남은 시간 %d"), GetName(), time);
}
...
void CItem::AccessorySocketDegrade()
{
	if (GetAccessorySocketGrade() > 0)
	{
		...
		if (ch)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "%s에 박혀있던 보석이 사라집니다."), GetName());
		}
