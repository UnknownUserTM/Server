...
namespace quest
{
	...
	int pet_send_exp_effect(lua_State* L)
	{
		CQuestManager& q = CQuestManager::instance();
		LPCHARACTER ch = q.GetCurrentCharacterPtr();
		LPCHARACTER npc = q.GetCurrentNPCCharacterPtr();
		CPetSystem* petSystem = ch->GetPetSystem();
		CPetActor* petActor = petSystem->GetByVID(npc->GetVID());	
		ch->CreateFly(FLY_SP_MEDIUM, petActor->GetCharacter());
		return 1;
		
	}
	
	int pet_remove_attribute(lua_State* L)
	{
		CQuestManager& q = CQuestManager::instance();
		LPCHARACTER ch = q.GetCurrentCharacterPtr();
		ch->RemovePetBonus();
	...
	int pet_set_attribute(lua_State* L)
	{
		CQuestManager& q = CQuestManager::instance();
		LPCHARACTER ch = q.GetCurrentCharacterPtr();
		ch->SetPetBonus(lua_tonumber(L,1),lua_tonumber(L,2),lua_tonumber(L,3));
		return 0;
	}	
	...
	void RegisterPetFunctionTable()
	{
		luaL_reg pet_functions[] =
		{
			...
			{ "is_mine", pet_is_mine },
			{ "send_exp_effect", pet_send_exp_effect },
			
			{ "remove_attribute", pet_remove_attribute },
			{ "set_attribute", pet_set_attribute },
