...
ACMD(do_emotion_allow)
{
	...
	{
		//ben blokladım hacı
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "%s blokkk"), tch->GetName());
		return;
	}
	if (MessengerManager::instance().IsBlocked_Target(ch->GetName(), tch->GetName()))
	{
		//o blokladı hacı
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "%s blokkk_me"), tch->GetName());
		return;
	}
	#endif	
...
ACMD(do_emotion)
{
	int i;
	{
		if (ch->IsRiding())
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "말을 탄 상태에서 감정표현을 할 수 없습니다."));
			return;
		}
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "열정의 가면을 착용시에만 할 수 있습니다."));
		return;
	}

	if (IS_SET(emotion_types[i].flag, WOMAN_ONLY) && SEX_MALE==GET_SEX(ch))
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "여자만 할 수 있습니다."));
		return;
	}
	...
	if (IS_SET(emotion_types[i].flag, NEED_TARGET | NEED_PC))
	{
		if (!victim)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "그런 사람이 없습니다."));
			return;
		}
	}
	...
	if (victim)
	{
		...
		if (victim->IsRiding())
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "말을 탄 상대와 감정표현을 할 수 없습니다."));
			return;
		}
		...
		if (distance < 10)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "너무 가까이 있습니다."));
			return;
		}

		if (distance > 500)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "너무 멀리 있습니다"));
			return;
		}
...
#ifdef ENABLE_NEWSTUFF
			if (GET_SEX(ch)==GET_SEX(victim) && !g_bEmotionSameGender)
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "이성간에만 할 수 있습니다."));
				return;
			}
#else
		...
		if (IS_SET(emotion_types[i].flag, NEED_PC))
		{
			if (s_emotion_set.find(std::make_pair(victim->GetVID(), ch->GetVID())) == s_emotion_set.end())
			{
				if (true == marriage::CManager::instance().IsMarried( ch->GetPlayerID() ))
				{
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "이 행동은 상호동의 하에 가능 합니다."));
						return;
					}
				}
				else
