...
void CHARACTER::StateIdle()
{
	if (IsStone())
	{
		...
		if(auto_purge_time > 1)
		{
			if(auto_purge_time < get_global_time())
			{
		...
		{
			int plague_time = quest::CQuestManager::instance().GetEventFlag("plague_event");
			if(plague_time<get_global_time())
			{
				M2_DESTROY_CHARACTER(this);
				return;				
			}
		...
		if(GetRaceNum() == 6650)
		{
			if(plague_evolution_time<get_global_time())
			{
				int dir = number(0,359);
				LPCHARACTER tch = CHARACTER_MANAGER::instance().SpawnMob(6651, GetMapIndex(), GetX(), GetY(), 0, false, dir);
				if (tch)
				{
					tch->plague_evolution_time = get_global_time() + 60;
				}	
				M2_DESTROY_CHARACTER(this);					
				return;				
			}
		...
		if(GetRaceNum() == 6651)
		{
			...
			{
				int dir = number(0,359);
				LPCHARACTER tch = CHARACTER_MANAGER::instance().SpawnMob(6652, GetMapIndex(), GetX(), GetY(), 0, false, dir);
				if (tch)
				{
					tch->plague_evolution_time = get_global_time() + 90;
				}	
				M2_DESTROY_CHARACTER(this);					
				return;				
			}
	...
	if (!IsMonster())
	{
		if(GetRaceNum() == 20080)
		{
			int plague_time = quest::CQuestManager::instance().GetEventFlag("burning_town");
			if(plague_time<get_global_time())
			{
				M2_DESTROY_CHARACTER(this);					
				return;					
...
void CHARACTER::__StateIdle_Portal()
{
	m_dwStateDuration = PASSES_PER_SEC(1);
	...
	if (iPercent <= 99 && GetMaxSP() < 1) // Phase 1
	{
		SetMaxSP(1);
		...
		for (int i = 0; i < 25; ++i)
		{	
			CHARACTER_MANAGER::instance().SpawnMobRange(number(101,104), GetMapIndex(), sx, sy, ex, ey, true, true);
		}
...
void CHARACTER::__StateIdle_Canon()
{

	LPDUNGEON pDungeon = CDungeonManager::instance().FindByMapIndex(GetMapIndex());
	if (pDungeon)
	{
		int bossVID = pDungeon->GetFlag("boss_vid");
		if(bossVID)
		{
			m_dwStateDuration = PASSES_PER_SEC(1);
			...
			if (iPercent <= 30 && GetMaxSP() < 8)
			{
				SetMaxSP(8);
				LPCHARACTER ch = CHARACTER_MANAGER::instance().Find((DWORD)bossVID);
				if(!ch)
				{
					sys_err("__StateIdle_Canon: invalid vid");
					return;
				}				
				ch->AddAffect(AFFECT_STUN, POINT_NONE, 0, AFF_STUN, 15, 0, false);
				SendMovePacket(FUNC_ATTACK, 0, GetX(), GetY(), 0);
				M2_DESTROY_CHARACTER(this);
			}
...
void CHARACTER::__StateIdle_Monster()
{
	...
	// if (GetRaceNum() == 6708)
	// {
		// if(ent_summon_mass_timer==0)
		// {
			// ent_summon_mass_timer = 1;
			// SendNoticeMap("Exterminatus: Erzittert sterbliche und verzweifel! Denn ich kehre zuruck...", GetMapIndex(), true);
		// }
	// }
...
void CHARACTER::StateMove()
{
	...
	else
	{
		...
		if (IsMonster() && GetMobRank() >= MOB_RANK_BOSS && GetVictim())
		{
			...
			// 거대 거북
			if (GetRaceNum() == 3491 && number(1, 20) == 1 && get_dword_time() - m_pkMobInst->m_dwLastWarpTime > 1000)
			{
				// 워프 테스트
				float fx, fy;
...
void CHARACTER::StateBattle()
{
	...
	if (GetRaceNum() == 3911)
	{
		if(ent_summon_mass_timer<get_global_time())
		{
			for (int i = 0; i < 5; ++i)
			{		
				if (!GetParty())
				...
				if(pParty->CountMemberByVnum(7024)<20)
				{
					int sx = GetX() - 500;
					int sy = GetY() - 500;
					int ex = GetX() + 500;
					...
					LPCHARACTER tch = CHARACTER_MANAGER::instance().SpawnMobRange(7024, GetMapIndex(), sx, sy, ex, ey, true, true);
					if (tch)
					{
						tch->auto_purge_time = get_global_time() + 30;
						pParty->Join(tch->GetVID());
						pParty->Link(tch);
					}						
				}
			}	
			ent_summon_mass_timer = get_global_time() + 5;	
		}
	}
	...
	if (GetRaceNum() == 6705)
	{
		if(ent_summon_mass_timer<get_global_time())
		{
			...
			{		
				...
				LPPARTY pParty = GetParty();
				if(pParty->CountMemberByVnum(6701)<20)
				{
					int sx = GetX() - 500;
					int sy = GetY() - 500;
					int ex = GetX() + 500;
					...
					LPCHARACTER tch = CHARACTER_MANAGER::instance().SpawnMobRange(6701, GetMapIndex(), sx, sy, ex, ey, true, true);
					if (tch)
					{
						tch->auto_purge_time = get_global_time() + 30;
						pParty->Join(tch->GetVID());
			...
			ent_summon_mass_timer = get_global_time() + 1;	
		}
	}
	// // En-Tai Exterminatus
	// if (GetRaceNum() == 6708)
	...
	// {
		...
		// {
			...
			// int low_hp = (GetMaxHP() * 10) / 100;
			// SetHP(low_hp);
			// if (iPercent <= 30 && GetMaxSP() < 1)
			// {
				// SetMaxSP(1);
				// for (int i = 0; i < 2; ++i)
				// {
					...
					// LPCHARACTER tch = CHARACTER_MANAGER::instance().SpawnMobRange(3491, GetMapIndex(), sx, sy, ex, ey, true, true);
					// tch->is_main_leviathan = false;
					// tch->SetHP(low_hp);
					// if (tch)
					// {
						// pParty->Join(tch->GetVID());
	...
	// {
		...
		// {
			...
			// {
				...
				// LPCHARACTER tch = CHARACTER_MANAGER::instance().SpawnMobRange(1191, GetMapIndex(), sx, sy, ex, ey, true, true);
		...
		// {
			...
			// {
				...
				// LPCHARACTER tch = CHARACTER_MANAGER::instance().SpawnMobRange(2491, GetMapIndex(), sx, sy, ex, ey, true, true);
	...
	if (GetRaceNum() == 2092)
	{
		...
		if (iPercent <= 5 && GetMaxSP() < 11)
		{
			SetMaxSP(11);
			CHARACTER_MANAGER::instance().SpawnGroup(number(2071,2075), GetMapIndex(), GetX() - 500, GetY() - 500, GetX() + 500, GetY() + 500);
			CHARACTER_MANAGER::instance().SpawnGroup(number(2071,2075), GetMapIndex(), GetX() - 500, GetY() - 500, GetX() + 500, GetY() + 500);
			CHARACTER_MANAGER::instance().SpawnGroup(number(2071,2075), GetMapIndex(), GetX() - 500, GetY() - 500, GetX() + 500, GetY() + 500);
			CHARACTER_MANAGER::instance().SpawnGroup(number(2071,2075), GetMapIndex(), GetX() - 500, GetY() - 500, GetX() + 500, GetY() + 500);
			CHARACTER_MANAGER::instance().SpawnGroup(number(2071,2075), GetMapIndex(), GetX() - 500, GetY() - 500, GetX() + 500, GetY() + 500);
		...
		else if (iPercent <= 10 && GetMaxSP() < 10)
		{
			SetMaxSP(10);
			CHARACTER_MANAGER::instance().SpawnGroup(number(2071,2075), GetMapIndex(), GetX() - 500, GetY() - 500, GetX() + 500, GetY() + 500);
			CHARACTER_MANAGER::instance().SpawnGroup(number(2071,2075), GetMapIndex(), GetX() - 500, GetY() - 500, GetX() + 500, GetY() + 500);
