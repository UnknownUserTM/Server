...
void FishingReact(LPCHARACTER ch)
{
	...
	// ch->ChatPacket(CHAT_TYPE_INFO, "FishingReact");
}
...
void FishingSuccess(LPCHARACTER ch)
{
	...
	p.info = ch->GetVID();
	ch->PacketAround(&p, sizeof(p));
	// ch->ChatPacket(CHAT_TYPE_INFO, "FishingSuccess");
}
...
void FishingFail(LPCHARACTER ch)
{
	...
	p.info = ch->GetVID();
	ch->PacketAround(&p, sizeof(p));
	// ch->ChatPacket(CHAT_TYPE_INFO, "FishingFail");
}
...
void FishingPractice(LPCHARACTER ch)
{
	...
	if (rod && rod->GetType() == ITEM_ROD)
	{
		...
		if ( rod->GetRefinedVnum()>0 && rod->GetSocket(0) < rod->GetValue(2) && number(1,rod->GetValue(1))==1 )
		{
			...
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "낚시대의 수련도가 증가하였습니다! (%d/%d)"),rod->GetSocket(0), rod->GetValue(2));
			if (rod->GetSocket(0) == rod->GetValue(2))
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "낚시대가 최대 수련도에 도달하였습니다."));
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "어부를 통해 다음 레벨의 낚시대로 업그레이드 할 수 있습니다."));
			}
		}
	}
	// 미끼를 뺀다
...
void Take(fishing_event_info* info, LPCHARACTER ch)
{
	if (info->step == 1)	// 고기가 걸린 상태면..
	{
		...
		switch (ret)
		{
			...
			case -3: // 난이도 때문에 실패
			case -1: // 시간 확률 때문에 실패
				//ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(//ch->GetLanguage(), "고기가 미끼만 빼먹고 잽싸게 도망칩니다."));
				{
					int map_idx = ch->GetMapIndex();
					int prob_idx = GetProbIndexByMapIndex(map_idx);
			...
			case 0:
				//ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(//ch->GetLanguage(), "고기가 잡혔습니다! (%s)"), fish_info[info->fish_id].name);
				if (item_vnum)
				{
					FishingSuccess(ch);
					...
					ch->SetQuestFlag("achievement_main.achievement_fish",ch->GetQuestFlag("achievement_main.achievement_fish")+1);
					int fishCount = ch->GetQuestFlag("random_quest.fish_count") - 1;
					if (fishCount <= 0)
						ch->SetQuestFlag("random_quest.fish_count",0);
					else
						ch->SetQuestFlag("random_quest.fish_count",fishCount);
					
					// ch->ChatPacket(CHAT_TYPE_INFO, "FishItemVnum: %s", item_vnum);
					...
					if (item)
					{
						...
						if (test_server)
						{
							ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "이번에 잡은 물고기의 길이는 %.2fcm"), item->GetSocket(0)/100.f);
						}
...
void Simulation(int level, int count, int prob_idx, LPCHARACTER ch)
{
	...
	for (std::map<std::string,int>::iterator it = fished.begin(); it != fished.end(); ++it)
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "%s : %d 마리"), it->first.c_str(), it->second);

	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "%d 종류 %d 마리 낚음"), fished.size(), total_count);
}
	...
	else
	{
		...
		switch (fish_info[idx].used_table[u_index])
		{
			...
			case USED_WATER_STONE:	// 2
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "고기가 흔적도 없이 사라집니다."));
				break;

			case USED_SHELLFISH:	// 1
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "배 속에서 조개가 나왔습니다."));
				ch->AutoGiveItem(SHELLFISH_VNUM);
				break;

			case USED_EARTHWARM:	// 4
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "배 속에서 지렁이가 나왔습니다."));
...
void Grill(LPCHARACTER ch, LPITEM item)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "%s를 구웠습니다."), item->GetName());
	item->SetCount(0);
...
int RealRefineRod(LPCHARACTER ch, LPITEM item)
{
	...
	else
	{
		...
		{
			// if (test_server) ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(// if (test_server) ch->GetLanguage(), "<FishRod> PRE %u"), rod->GetSocket(0));
			int cur = rod->GetSocket(0);
			rod->SetSocket(0, (cur > 0) ? (cur - (cur * 10 / 100)) : 0);
			// if (test_server) ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(// if (test_server) ch->GetLanguage(), "<FishRod> POST %u"), rod->GetSocket(0));
			// ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(// ch->GetLanguage(), "<FishRod> The upgrade has failed, and the fishrod has lost 10%% of its mastery points."));
