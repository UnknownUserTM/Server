...
void DBManager::SendAuthLogin(LPDESC d)
{
	...
	strlcpy(ptod.szLanguage, r.szLanguage, sizeof(ptod.szLanguage));
					...
					{
						{
							...
							char szQuery[1024];
							snprintf(szQuery, sizeof(szQuery), "UPDATE account SET last_play=NOW(), language='%s' WHERE id=%u", pinfo->szLanguage, dwID);
						...
						strlcpy(r.szLanguage, pinfo->szLanguage, sizeof(r.szLanguage));
				...
				if (ch)
				{
					...
					else
					{
						...
						if (pkItem)
						{
							...
							//ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(//ch->GetLanguage(), "아이템 획득: %s"), pkItem->GetName());
...
void DBManager::SendMoneyLog(BYTE type, DWORD vnum, int64_t gold)
{
	if (!gold)
		return;
	TPacketMoneyLog p;
...
void VCardUse(LPCHARACTER CardOwner, LPCHARACTER CardTaker, LPITEM item)
{
	...
	CardTaker->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(CardTaker->GetLanguage(), "%d분의 결제시간이 추가 되었습니다. (결제번호 %d)"), item->GetSocket(1) / 60, item->GetSocket(0));
