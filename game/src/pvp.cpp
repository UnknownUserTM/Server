...
void CPVPManager::Insert(LPCHARACTER pkChr, LPCHARACTER pkVictim)
{
	...
	if ((pkPVP = Find(kPVP.m_dwCRC)))
	{
		...
		{
			pkVictim->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pkVictim->GetLanguage(), "%s님과의 대결 시작!"), pkChr->GetName());
			pkChr->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pkChr->GetLanguage(), "%s님과의 대결 시작!"), pkVictim->GetName());
		}
		return;
	}
	...
	pkChr->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pkChr->GetLanguage(), "%s에게 대결신청을 했습니다."), pkVictim->GetName());
