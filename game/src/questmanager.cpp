...
namespace quest
{
	...
	bool CQuestManager::Initialize()
	{
		...
		m_mapEventName.insert(TEventNameMap::value_type("hit", QUEST_HIT_EVENT));	// 독일선물기능테스트
...
// #define ENABLE_PARTYKILL
	...
	void CQuestManager::QuestInfo(unsigned int pc, unsigned int quest_index)
	{
		...
		if ((pPC = GetPC(pc)))
		{
			...
			if (!CheckQuestLoaded(pPC))
			{
				...
				if (ch)
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "퀘스트를 로드하는 중입니다. 잠시만 기다려 주십시오."));
    ...
    void CQuestManager::hit(unsigned int pc)
    {
        PC * pPC;
