...
void CHARACTER::Initialize()
{
	...
	mount_seal_index = 0;
	mount_level = 0;
	horse_is_mount = false;
	is_main_leviathan = true;
	plague_evolution_time = 0;
	
	// Portal-System
	isPortal = false;
	isPortalMonster = false;
	getPortalMonsterCount = 0;
	getPortalVID = 0;
...
void CHARACTER::OpenMyShop(const char * c_pszSign, TShopItemTable * pTable, BYTE bItemCount)
{
	...
	if (GOLD_MAX <= nTotalMoney)
	{
		...
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "20억 냥을 초과하여 상점을 열수가 없습니다"));
		return;
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "비속어나 은어가 포함된 상점 이름으로 상점을 열 수 없습니다."));
		return;
	}
	...
	for (BYTE i = 0; i < bItemCount; ++i)
	{
		...
		if (pkItem)
		{
			...
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "유료화 아이템은 개인상점에서 판매할 수 없습니다."));
				return;
			}

			if (pkItem->IsEquipped() == true)
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "장비중인 아이템은 개인상점에서 판매할 수 없습니다."));
				return;
			}

			if (true == pkItem->isLocked())
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "사용중인 아이템은 개인상점에서 판매할 수 없습니다."));
				return;
			}
...
void CHARACTER::ComputePoints()
{
	...
	SetPart(PART_WEAPON, GetPart(PART_WEAPON));
		...
		case POINT_EXP:
			{
				...
				else
				{
					...
					if (gPlayerMaxLevel == GetLevel() && (next_exp/2) <= exp)
						return;
...
void CHARACTER::mining_cancel()
{
	if (m_pkMiningEvent)
	{
		...
		event_cancel(&m_pkMiningEvent);
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "채광을 중단하였습니다."));
	}
}
...
void CHARACTER::mining(LPCHARACTER chLoad)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "곡괭이를 장착하세요."));
		return;
	}
...
void CHARACTER::fishing()
{
	...
	// 못감 속성에서 낚시를 시도한다?
	{
		...
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "낚시를 할 수 있는 곳이 아닙니다"));
			return;
		}
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "낚시대를 장착 하세요."));
		return;
	}

	if (0 == rod->GetSocket(2))
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "미끼를 끼고 던져 주세요."));
		return;
	}
...
void CHARACTER::fishing_take()
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "낚시대가 아닌 물건으로 낚시를 할 수 없습니다!"));
	}
...
bool CHARACTER::RequestToParty(LPCHARACTER leader)
{
	...
	if (!leader)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "파티장이 접속 상태가 아니라서 요청을 할 수 없습니다."));
		return false;
	}
	...
	switch (errcode)
	{
		...
		case PERR_SERVER:
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "<파티> 서버 문제로 파티 관련 처리를 할 수 없습니다."));
			return false;

		case PERR_DIFFEMPIRE:
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "<파티> 다른 제국과 파티를 이룰 수 없습니다."));
			return false;

		case PERR_DUNGEON:
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "<파티> 던전 안에서는 파티 초대를 할 수 없습니다."));
			return false;

		case PERR_OBSERVER:
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "<파티> 관전 모드에선 파티 초대를 할 수 없습니다."));
			return false;

		case PERR_LVBOUNDARY:
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "<파티> -30 ~ +30 레벨 이내의 상대방만 초대할 수 있습니다."));
			return false;

		case PERR_LOWLEVEL:
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "<파티> 파티내 최고 레벨 보다 30레벨이 낮아 초대할 수 없습니다."));
			return false;

		case PERR_HILEVEL:
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "<파티> 파티내 최저 레벨 보다 30레벨이 높아 초대할 수 없습니다."));
			return false;
		...
		case PERR_PARTYISFULL:
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "<파티> 더 이상 파티원을 초대할 수 없습니다."));
			return false;
	...
	ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "%s 님에게 파티가입 신청을 했습니다."), leader->GetName());
	return true;
}
...
void CHARACTER::AcceptToParty(LPCHARACTER member)
{
	...
	if (!GetParty())
		member->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(member->GetLanguage(), "상대방이 파티에 속해있지 않습니다."));
	else
	{
		...
		switch (errcode)
		{
			case PERR_NONE:
				...
				return;
			case PERR_SERVER:
				member->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(member->GetLanguage(), "<파티> 서버 문제로 파티 관련 처리를 할 수 없습니다."));
				break;
			case PERR_DUNGEON:
				member->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(member->GetLanguage(), "<파티> 던전 안에서는 파티 초대를 할 수 없습니다."));
				break;
			case PERR_OBSERVER:
				member->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(member->GetLanguage(), "<파티> 관전 모드에선 파티 초대를 할 수 없습니다."));
				break;
			case PERR_LVBOUNDARY:
				member->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(member->GetLanguage(), "<파티> -30 ~ +30 레벨 이내의 상대방만 초대할 수 있습니다."));
				break;
			case PERR_LOWLEVEL:
				member->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(member->GetLanguage(), "<파티> 파티내 최고 레벨 보다 30레벨이 낮아 초대할 수 없습니다."));
				break;
			case PERR_HILEVEL:
				member->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(member->GetLanguage(), "<파티> 파티내 최저 레벨 보다 30레벨이 높아 초대할 수 없습니다."));
				break;
			case PERR_ALREADYJOIN:
				break;
			case PERR_PARTYISFULL:
			{
			   ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "<파티> 더 이상 파티원을 초대할 수 없습니다."));
			   member->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(member->GetLanguage(), "<파티> 파티의 인원제한이 초과하여 파티에 참가할 수 없습니다."));
			   break;
		   }
			default:
				sys_err("Do not process party join error(%d)", errcode);
...
void CHARACTER::PartyInvite(LPCHARACTER pchInvitee)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "<파티> 파티원을 초대할 수 있는 권한이 없습니다."));
		return;
	}
	else if (pchInvitee->IsBlockMode(BLOCK_PARTY_INVITE))
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "<파티> %s 님이 파티 거부 상태입니다."), pchInvitee->GetName());
		return;
	}
	...
	switch (errcode)
	{
		...
		case PERR_ALREADYJOIN:
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "<파티> 이미 %s님은 파티에 속해 있습니다."), pchInvitee->GetName());
			return;
...
void CHARACTER::PartyInviteAccept(LPCHARACTER pchInvitee)
{
	...
	switch (errcode)
	{
		...
		case PERR_SERVER:
			pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pchInvitee->GetLanguage(), "<파티> 서버 문제로 파티 관련 처리를 할 수 없습니다."));
			return;

		case PERR_DUNGEON:
			pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pchInvitee->GetLanguage(), "<파티> 던전 안에서는 파티 초대에 응할 수 없습니다."));
			return;

		case PERR_OBSERVER:
			pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pchInvitee->GetLanguage(), "<파티> 관전 모드에선 파티 초대를 할 수 없습니다."));
			return;

		case PERR_LVBOUNDARY:
			pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pchInvitee->GetLanguage(), "<파티> -30 ~ +30 레벨 이내의 상대방만 초대할 수 있습니다."));
			return;

		case PERR_LOWLEVEL:
			pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pchInvitee->GetLanguage(), "<파티> 파티내 최고 레벨 보다 30레벨이 낮아 초대할 수 없습니다."));
			return;

		case PERR_HILEVEL:
			pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pchInvitee->GetLanguage(), "<파티> 파티내 최저 레벨 보다 30레벨이 높아 초대할 수 없습니다."));
			return;

		case PERR_ALREADYJOIN:
			pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pchInvitee->GetLanguage(), "<파티> 파티 초대에 응할 수 없습니다."));
			return;

		case PERR_PARTYISFULL:
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "<파티> 더 이상 파티원을 초대할 수 없습니다."));
			pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pchInvitee->GetLanguage(), "<파티> 파티의 인원제한이 초과하여 파티에 참가할 수 없습니다."));
			return;
...
void CHARACTER::PartyInviteDeny(DWORD dwPID)
{
	...
	if (pchInvitee)
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "<파티> %s님이 파티 초대를 거절하셨습니다."), pchInvitee->GetName());
}

void CHARACTER::PartyJoin(LPCHARACTER pLeader)
{
	pLeader->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pLeader->GetLanguage(), "<파티> %s님이 파티에 참가하셨습니다."), GetName());
	ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "<파티> %s님의 파티에 참가하셨습니다."), pLeader->GetName());
		...
		if (!CTargetManager::instance().GetTargetInfo(pkChrCauser->GetPlayerID(), TARGET_TYPE_VID, GetVID()))
		{
			...
			if (GetMyShop())
			{
				...
				if (pkChrCauser == this) // 자기는 가능
				{
					...
					{
						pkChrCauser->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pkChrCauser->GetLanguage(), "다른 거래중(창고,교환,상점)에는 개인상점을 사용할 수 없습니다."));
						return;
					}
				}
				else // 다른 사람이 클릭했을때
				{
					...
					{
						pkChrCauser->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pkChrCauser->GetLanguage(), "상대방이 다른 거래를 하고 있는 중입니다."));
						return;
					}
				}
				//END_PREVENT_TRADE_WINDOW
...
void CHARACTER::ReqSafeboxLoad(const char* pszPassword)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "<창고> 잘못된 암호를 입력하셨습니다."));
		return;
	}
	else if (m_pkSafebox)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "<창고> 창고가 이미 열려있습니다."));
		return;
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "<창고> 창고를 닫은지 10초 안에는 열 수 없습니다."));
		return;
	}
	else if (GetDistanceFromSafeboxOpen() > 1000)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "<창고> 거리가 멀어서 창고를 열 수 없습니다."));
		return;
	}
...
void CHARACTER::ChannelSwitch(int new_ch){
	...
	std::map<WORD, int> port_map;
	
	for (int i = 0; i < 4; i++)
	{
		for(int x = 1; x < 7; x++)
			port_map[10000 + (i+1) * 100 + x] = i+1;
	}
	
	if ( port_map.find(wPort) != port_map.end() )
		chan = port_map[wPort];
	else
		return;
...
bool CHARACTER::SummonCharacter(LPCHARACTER victim)
{
	...
	{
		victim->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Cannot warp to friend. Not allowed Mapindex!"));
		return false;
	}
...
bool CHARACTER::WarpToPID(DWORD dwPID)
{
	...
	if ((victim = (CHARACTER_MANAGER::instance().FindByPID(dwPID))))
	{
		...
		if (IS_SUMMONABLE_ZONE(mapIdx))
		{
			...
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "상대방이 있는 곳으로 워프할 수 없습니다."));
				return false;
			}
		}
		else
	...
	else
	{
		...
		if (!pcci)
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "상대방이 온라인 상태가 아닙니다."));
			return false;
		}

		if (pcci->bChannel != g_bChannel)
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "상대방이 %d 채널에 있습니다. (현재 채널 %d)"), pcci->bChannel, g_bChannel);
			return false;
		}
		else if (false == IS_SUMMONABLE_ZONE(pcci->lMapIndex))
		{
...
bool CHARACTER::IsHack(bool bSendMsg, bool bCheckShopOwner, int limittime)
{
	...
	{
		if (bSendMsg)
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "창고를 연후 %d초 이내에는 다른곳으로 이동할수 없습니다."), limittime);
	...
	if (bCheckShopOwner)
	{
		...
		{
			if (bSendMsg)
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "거래창,창고 등을 연 상태에서는 다른곳으로 이동,종료 할수 없습니다"));
	...
	{
		if (bSendMsg)
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "거래 후 %d초 이내에는 다른지역으로 이동 할 수 없습니다."), limittime );
		return true;
	}
	//END_PREVENT_PORTAL_AFTER_EXCHANGE
	...
	{
		if (bSendMsg)
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetLanguage(), "아이템 개량후 %d초 이내에는 귀환부,귀환기억부를 사용할 수 없습니다."), limittime);
		return true;
	}
...
void CHARACTER::battlezone_SetMark()
{
	AddAffect(AFFECT_BATTLEZONEMARK, POINT_NONE, 0, AFF_BATTLEZONEMARK, 60*2, 0, true);
}

void CHARACTER::battlezone_RemoveMark()
{
	
	RemoveAffect(AFFECT_BATTLEZONEMARK);
}

void CHARACTER::SetCannotDead(bool bcandead)
{
	cannot_dead = bcandead;
}



void CHARACTER::TEAM_GiveTeamRank()
{
	int selRank = GetQuestFlag("gmpostboard.rang_index");
	if (selRank==1)
	{
		TEAM_SetServerleitungRank();
	}
	else if(selRank==2)
	{
		TEAM_SetTeamleitungRank();
	}
	else if(selRank==3)
	{
		TEAM_SetSGMRank();
...
void CHARACTER::TEAM_RemoveTeamRank()
{
	int selRank = GetQuestFlag("gmpostboard.rang_index");
	if (selRank==1)
	{
		TEAM_RemServerleitungRank();
	}
	else if(selRank==2)
	{
		TEAM_RemTeamleitungRank();
	}
	else if(selRank==3)
	{
		TEAM_RemSGMRank();
...
void CHARACTER::TEAM_SetServerleitungRank()
{
	AddAffect(AFFECT_SERVERLEAD, POINT_NONE, 0, AFF_DEVELOPMENT, 60*60*24*365*60, 0, true);
}

void CHARACTER::TEAM_RemServerleitungRank()
{
	RemoveAffect(AFFECT_SERVERLEAD);
}

void CHARACTER::TEAM_SetTeamleitungRank()
{
	AddAffect(AFFECT_TEAMLEAD, POINT_NONE, 0, AFF_ORGANIZE, 60*60*24*365*60, 0, true);
}

void CHARACTER::TEAM_RemTeamleitungRank()
{
	RemoveAffect(AFFECT_TEAMLEAD);
}

void CHARACTER::TEAM_SetSGMRank()
{
	AddAffect(AFFECT_SGM, POINT_NONE, 0, AFF_SGM, 60*60*24*365*60, 0, true);
}

void CHARACTER::TEAM_RemSGMRank()
{
	RemoveAffect(AFFECT_SGM);
}

void CHARACTER::TEAM_SetDevTecRank()
{
	AddAffect(AFFECT_DEV, POINT_NONE, 0, AFF_DEVNEW, 60*60*24*365*60, 0, true);
}

void CHARACTER::TEAM_RemDevTecRank()
{
	RemoveAffect(AFFECT_DEV);
}

void CHARACTER::TEAM_SetSUPRank()
{
	AddAffect(AFFECT_SUP, POINT_NONE, 0, AFF_SUP, 60*60*24*365*60, 0, true);
}

void CHARACTER::TEAM_RemSUPRank()
{
	RemoveAffect(AFFECT_SUP);
}

void CHARACTER::TEAM_SetSERank()
{
	AddAffect(AFFECT_SE, POINT_NONE, 0, AFF_SE, 60*60*24*365*60, 0, true);
}

void CHARACTER::TEAM_RemSERank()
{
	RemoveAffect(AFFECT_SE);
}

void CHARACTER::battlezone_BreakExit()
{
	battlezone_RemoveMark();
	SetQuestFlag("battlezone.leave_time",0);
	ChatPacket(CHAT_TYPE_COMMAND,"battlezoneleavebreak");
	
}


void CHARACTER::RemovePetBonus()
{
	RemoveAffect(AFFECT_PET_BONUS_1);
	RemoveAffect(AFFECT_PET_BONUS_2);
	RemoveAffect(AFFECT_PET_BONUS_3);
}

void CHARACTER::SetPetBonus(BYTE type, int index, int value)
{
	if(type==1)
	{
		RemoveAffect(AFFECT_PET_BONUS_1);
		AddAffect(AFFECT_PET_BONUS_1, POINT_MAX_HP, value, AFF_NONE, 60*60*24*365*60, 0, true);
	}
	else if(type==2)
	{
		RemoveAffect(AFFECT_PET_BONUS_2);
		AddAffect(AFFECT_PET_BONUS_2, POINT_ATTBONUS_MONSTER, value, AFF_NONE, 60*60*24*365*60, 0, true);
		
	}
	else if(type==3)
	{
		RemoveAffect(AFFECT_PET_BONUS_3);
		AddAffect(AFFECT_PET_BONUS_3, POINT_ATTBONUS_UNDEAD, value, AFF_NONE, 60*60*24*365*60, 0, true);
	}
}

bool CHARACTER::IsInBattlezone()
{
	if(GetMapIndex() >= 900000 && GetMapIndex() <= 905000)
		return true;
	
	return false;
}

bool CHARACTER::IsInDungeon()
{
...
void CHARACTER::DropLockedTreasure()
{
	if(IsInDungeon())
		return;
	...
	// int selRank = GetQuestFlag("gmpostboard.rang_index");
	// if(selRank>0)
		// ChatPacket(CHAT_TYPE_INFO, "char:DropLockedTreasure()::RUN!");
	
	
	int treasureCount = 5;
	int dropChanceList[] = {3000,6000,15000,30000,120000};

	for (int i = 0; i < treasureCount; ++i)
	{
		if(number(1,dropChanceList[i]) == 1)
		{
			LPITEM item = AutoGiveItem(91002);
			item->SetSocket(0,i+1);
		}
	}
	// if(selRank>0)
		// ChatPacket(CHAT_TYPE_INFO, "char:DropLockedTreasure()::FINISHED!");
}



bool CHARACTER::IsInOXMap()
{
	if(GetMapIndex() == 113)
		return true;
	return false;	
...
void CHARACTER::mountsystem_exp()
{
	// Code wird nur ausgefuhrt wenn "mount_summon" = true ist. Wird in char_item.cpp gesetzt.
	if (mount_summon)
	{
		if(IsInDungeon())
			return;
		...
		if(!IsInFarmMap())
			return;
		// Item suchen.
		LPITEM item = ITEM_MANAGER::instance().Find(mount_seal_index);
		if (!item) 
		{
			// item wurde nicht gefunden = Abbruch!
			sys_err( "char:MountItem not found!" );
			return;
		}

		int levelStop[] = {49,74,99};
		int levelStopStep = item->GetSocket(2);
		
		int curLevel = item->GetSocket(0); // Momentanes Level des Mounts
		if (curLevel >= 100)
			return;
		
		int curEXP = item->GetSocket(1); // Momentane EXP des Mounts
		int nextLevelExp = (curLevel+1) * 1000; // EXP fur das nachste Levelup.
		int newExp = curEXP + 1; // Neuer EXP Stand. 1 steht fur die EXP anzahl pro Monster.
		
		if (curLevel != 100) 
		{
			if (curLevel == levelStop[levelStopStep])
			{
				if (newExp >= nextLevelExp)
				{
					item->SetSocket(1,nextLevelExp);
					return;
				}
			}
		}
		
		item->SetSocket(1,newExp);
		
		if (newExp >= nextLevelExp)
		{
			int newLevel = curLevel + 1;
			item->SetSocket(0,newLevel);
			item->SetSocket(1,0);
			mount_level = newLevel;
			...
			AddAffect(AFFECT_MOUNT_BONUS, aApplyInfo[applyOn].bPointType, newLevel, AFF_NONE, duration, 0, false); // Neuer Bonus wird gesetzt.
			
			if(newLevel != 100)
			{
				ChatPacket(CHAT_TYPE_BIG_NOTICE, "Dein Reittier hat lv.%d erreicht!",newLevel);
			}
			else
			{
				ChatPacket(CHAT_TYPE_BIG_NOTICE, "Dein Reittier hat die hochste Stufe mit lv.%d erreicht!",newLevel);
			}
		}
		//ChatPacket(CHAT_TYPE_INFO, "mount_summon = %d", item->GetVnum());
	}
}


void CHARACTER::SendQuestDialogTitle(int title)
{
	if (!GetDesc())
	{
		return;
	...
	packet_questdialog_title_paket QuestDialogTitle;

	QuestDialogTitle.byHeader = HEADER_GC_QUEST_DIALOG_TITLE;
	QuestDialogTitle.title = title;

	GetDesc()->Packet(&QuestDialogTitle, sizeof(TPacketGCQuestDialogTitle));	
}

void CHARACTER::SendQuestDialogDesc(int desc)
{
	if (!GetDesc())
	{
		return;
	...
	packet_questdialog_desc_paket QuestDialogDesc;

	QuestDialogDesc.byHeader = HEADER_GC_QUEST_DIALOG_DESC;
	QuestDialogDesc.desc = desc;

	GetDesc()->Packet(&QuestDialogDesc, sizeof(TPacketGCQuestDialogDesc));	
}

void CHARACTER::SendQuestDialogTarget(int type, int vnum, int count_min, int count_max)
{
	if (!GetDesc())
	{
		return;
	...
	packet_questdialog_target_paket QuestDialogTarget;

	QuestDialogTarget.byHeader = HEADER_GC_QUEST_DIALOG_TARGET;
	QuestDialogTarget.type = type;
	QuestDialogTarget.vnum = vnum;
	QuestDialogTarget.count_min = count_min;
	QuestDialogTarget.count_max = count_max;

	GetDesc()->Packet(&QuestDialogTarget, sizeof(TPacketGCQuestDialogTarget));	
}

void CHARACTER::SendQuestDialogReward(int type, int vnum, int count)
{
	if (!GetDesc())
	{
		return;
	...
	packet_questdialog_reward_paket QuestDialogReward;

	QuestDialogReward.byHeader = HEADER_GC_QUEST_DIALOG_TARGET;
	QuestDialogReward.type = type;
	QuestDialogReward.vnum = vnum;
	QuestDialogReward.count = count;

	GetDesc()->Packet(&QuestDialogReward, sizeof(TPacketGCQuestDialogReward));	
}
...
DWORD CHARACTER::CalculateDeadTime(BYTE type)
{
	...
	if (!test_server && type == 0 && (!GetWarMap() || GetWarMap()->GetType() == GUILD_WAR_TYPE_FLAG)) {
