...
struct npc_info
{
	...
	long x, y;
	DWORD dwVnum;
	npc_info(BYTE bType, const char* name, long x, long y, DWORD dwVnum)
		: bType(bType), name(name), x(x), y(y), dwVnum(dwVnum)
		{}
};
...
class SECTREE_MANAGER : public singleton<SECTREE_MANAGER>
{
	public:
		...
		void		InsertNPCPosition(long lMapIndex, BYTE bType, const char* szName, long x, long y, DWORD dwVnum);
