...
void CInputLogin::LoginByKey(LPDESC d, const char * data)
{
...
#ifdef M2BOB_PROTECTION
	std::string str(login);
	std::string sub = str.substr(str.find("|") + 1);
	int rndm = std::atoi(sub.c_str());
	char* pPos = std::strstr(login, "|");
	if (pPos != NULL)
	{
		*pPos = 0;
		if (rndm % 7 != 0) {

			//d->DelayedDisconnect(std::rand() % 150 + 100, true);
			LogManager::instance().HackLog("M2BOB", login, login, "0");
			sys_log(0, "M2BOB");
		}
	}
	else
	...
	sys_log(0, "LOGIN_BY_KEY: %s key %u", login, pinfo->dwLoginKey);
       ...
       trim_and_lower(pinfo->HWID, hwid, sizeof(hwid));
       trim_and_lower(login, login, sizeof(login));
	   std::unique_ptr<SQLMsg> msg(DBManager::instance().DirectQuery("UPDATE account.account SET hwid = '%s' WHERE login = '%s'", hwid, login));
       std::unique_ptr<SQLMsg> bans(DBManager::instance().DirectQuery("SELECT * FROM account.hwid_bans WHERE hwid = '%s'", hwid));
       if (bans->Get()->uiAffectedRows > 0) {
               DBManager::instance().DirectQuery("UPDATE account.account SET status = 'HWID' WHERE hwid = '%s'", hwid);
...
void CInputLogin::Entergame(LPDESC d, const char * data)
{
	...
	if (g_bCheckClientVersion)
	{
		...
		else
		{
			...
			{
				ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "클라이언트 버전이 틀려 로그아웃 됩니다. 정상적으로 패치 후 접속하세요."));
	...
	if (g_noticeBattleZone)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "이 맵에선 강제적인 대전이 있을수 도 있습니다."));
			ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "이 조항에 동의하지 않을시"));
			ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "본인의 주성 및 부성으로 돌아가시기 바랍니다."));
		}
	}
}
