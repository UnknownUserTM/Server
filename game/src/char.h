...
#endif
	POINT_ATTBONUS_STONE = 145,
	POINT_ATTBONUS_BOSS = 146,
	POINT_DEFBONUS_MONSTER = 147,
	POINT_ATTBONUS_PLAYER = 148,
	POINT_DEFBONUS_PLAYER = 149,
...
class CHARACTER : public CEntity, public CFSM, public CHorseRider
{
	...
	protected:
		...
		void				__StateIdle_Canon();
		void				__StateIdle_Portal();
		bool				__IsPortal();
...
//		bool			IsPet() const		{ return m_bCharType == CHAR_TYPE_PET; }
		int				last_mount_pet_costume_use;
		
		int				petSealItemIndex;
		
		int 			get_fire_bucket_count;
		int				auto_purge_time;
		int				ent_summon_mass_timer;
		bool			is_main_leviathan;
		int				plague_evolution_time;
		
		bool			isPortal;
		bool			isPortalMonster;
		DWORD			getPortalVID;
		int				getPortalMonsterCount;
		...
		bool			IsInFarmMap();
		bool			IsInOXMap();
		void			DropLockedTreasure();
		void			mountsystem_exp();
		void			battlezone_SetMark();
		...
		// QuestSystem //
		// QuestIntro ---------------------------------------------------------------------------- //
		void			SendQuestDialogTitle(int title);
		void			SendQuestDialogDesc(int desc);
		void			SendQuestDialogTarget(int type, int vnum, int count_min, int count_max);
		void			SendQuestDialogReward(int type, int vnum, int count);
		// MainQuest ----------------------------------------------------------------------------- //
		...
		void			SetPetBonus(BYTE type, int index, int value);
		void			RemovePetBonus();
		
		void			SetCannotDead(bool bcandead);
		
		// void			TEAM_SetDeveloperRank();
		// void			TEAM_RemDeveloperRank();
		// void			TEAM_SetOrganizeRank();
		// void			TEAM_RemOrganizeRank();
		// void			TEAM_SetSupportRank();
		// void			TEAM_RemSupportRank();
		
		void			TEAM_SetServerleitungRank();
		void			TEAM_RemServerleitungRank();
		
		void			TEAM_SetTeamleitungRank();
		void			TEAM_RemTeamleitungRank();
		
		void			TEAM_SetSGMRank();
		void			TEAM_RemSGMRank();
		
		void			TEAM_SetDevTecRank();
		void			TEAM_RemDevTecRank();
		
		void			TEAM_SetSUPRank();
		void			TEAM_RemSUPRank();
		
		void			TEAM_SetSERank();
		void			TEAM_RemSERank();
		
		
		void			TEAM_GiveTeamRank();
		void			TEAM_RemoveTeamRank();
		
		//void			mountsystem_summon();
		int				mount_level;
	...
	public:
		...
		bool			CanUnequipNow(const LPITEM item, bool check_costume = true);
		...
		bool			ItemIsUppStorageItem(DWORD vnum);
		int				CountItemInUppStorage(DWORD vnum);
		void			RemoveItemInUppStorage(DWORD vnum, DWORD count);
	...
	public:
		...
		void			GiveGold(int64_t iAmount);	// ????? ?????? ??? ???, ?a? ???? ??
		// End of Money
...
//		AttackLog			m_kAttackLog;
	...
	public:
		...
		void				LanguageChatPacket(BYTE bType, const char * format, ...);
		void				PacketNotifyPickup(const char * szBuf, DWORD dwVnum);

		void				SendShoutFilterListPacket();
		bool				GetOptionFilterChatByLanguage(const char * szLanguageSender);
		void				FilterChatPacket(const char * szNameSender, const char * szLanguageSender, BYTE bEmpireSender, const char * szTextLine);
		int					GetShoutLanguageByID(DWORD bLanguageID);
		void				SetShoutLanguageByID(BYTE bLanguageID, BYTE bValue);
		void				SetShoutLanguageGlobal();
