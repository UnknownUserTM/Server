...
namespace quest
{
	...
	int _debug(lua_State* L)
	{
		ostringstream s;
		combine_lua_string(L, s);
		CQuestManager::Instance().GetCurrentCharacterPtr()->ChatPacket(CHAT_TYPE_DEBUG, "%s", s.str().c_str());
		return 0;
	}
	...
	int _get_language(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		lua_pushstring(L, (ch) ? ch->GetLanguage() : g_stDefaultLanguage);
		return 1;
	}

	int _add_ox_quiz(lua_State* L)
	{
		const char * pszLanguage = lua_tostring(L, 1);
		const char * pszQuestion = lua_tostring(L, 2);
		bool bAnswer = lua_toboolean(L, 3);

		if (!COXEventManager::instance().AddQuiz(pszLanguage, pszQuestion, bAnswer))
			sys_log(0, "OXEVENT : Cannot add quiz. %s %s %d", pszLanguage, pszQuestion, bAnswer);
