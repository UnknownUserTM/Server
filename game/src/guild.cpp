...
bool CGuild::OfferExp(LPCHARACTER ch, int amount)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "<길드> 제공하고자 하는 경험치가 남은 경험치보다 많습니다."));
		return false;
	}
...
void CGuild::DeleteComment(LPCHARACTER ch, DWORD comment_id)
{
	...
	if (pmsg->Get()->uiAffectedRows == 0 || pmsg->Get()->uiAffectedRows == (uint32_t)-1)
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "<길드> 삭제할 수 없는 글입니다."));
	else
...
void CGuild::UseSkill(DWORD dwVnum, LPCHARACTER ch, DWORD pid)
{
	...
	if (GetSP() < iNeededSP)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "<길드> 용신력이 부족합니다. (%d, %d)"), GetSP(), iNeededSP);
		return;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "<길드> 쿨타임이 끝나지 않아 길드 스킬을 사용할 수 없습니다."));
		return;
	}
	...
	if (test_server)
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "<길드> %d 스킬을 사용함 (%d, %d) to %u"), dwVnum, GetSP(), iNeededSP, pid);
		...
		case GUILD_SKILL_TELEPORT:
			...
			else
			{
				if (m_memberP2POnline.find(pid) != m_memberP2POnline.end())
				{
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "<길드> 상대가 %d 채널에 있습니다. (현재 채널 %d)"), pcci->bChannel, g_bChannel);
					}
					else
					{
					...
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "<길드> 상대가 온라인 상태가 아닙니다."));
			}
			break;
		...
		default:
			{
				/*if (ch->GetPlayerID() != GetMasterPID())
				  {
				  ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "<길드> 길드장만 길드 스킬을 사용할 수 있습니다."));
				  return;
				  }*/

				if (!UnderAnyWar())
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "<길드> 길드 스킬은 길드전 중에만 사용할 수 있습니다."));
					return;
				}
...
bool CGuild::ChargeSP(LPCHARACTER ch, int iSP)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "<길드> %u의 용신력을 회복하였습니다."), iSP);
	}
	return true;
}
...
void CGuild::RequestDepositMoney(LPCHARACTER ch, int iGold)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "<길드> 잠시후에 이용해주십시오"));
		return;
	}
...
void CGuild::RequestWithdrawMoney(LPCHARACTER ch, int iGold)
{
	...
	if (ch->GetPlayerID() != GetMasterPID())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "<길드> 길드 금고에선 길드장만 출금할 수 있습니다."));
		return;
	}
...
void CGuild::Invite( LPCHARACTER pchInviter, LPCHARACTER pchInvitee )
{
	...
	{
	    pchInviter->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pchInviter->GetLanguage(), "<길드> 상대방이 초대 신청을 받을 수 없는 상태입니다."));
	    return;
	}
	...
	{
		pchInviter->ChatPacket( CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pchInviter->GetLanguage(), "<길드> 상대방이 길드 초대 거부 상태입니다.") );
		return;
	}
	else if ( !HasGradeAuth( GetMember( pchInviter->GetPlayerID() )->grade, GUILD_AUTH_ADD_MEMBER ) )
	{
		pchInviter->ChatPacket( CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pchInviter->GetLanguage(), "<길드> 길드원을 초대할 권한이 없습니다.") );
		return;
	}
	else if ( pchInvitee->GetEmpire() != pchInviter->GetEmpire() )
	{
		pchInviter->ChatPacket( CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pchInviter->GetLanguage(), "<길드> 다른 제국 사람을 길드에 초대할 수 없습니다.") );
		return;
	}
	...
	switch ( errcode )
	{
		...
		case GERR_WITHDRAWPENALTY:
			pchInviter->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pchInviter->GetLanguage(), "<길드> 탈퇴한 후 %d일이 지나지 않은 사람은 길드에 초대할 수 없습니다."), quest::CQuestManager::instance().GetEventFlag("guild_withdraw_delay"));
			return;
		case GERR_COMMISSIONPENALTY:
			pchInviter->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pchInviter->GetLanguage(), "<길드> 길드를 해산한 지 %d일이 지나지 않은 사람은 길드에 초대할 수 없습니다."), quest::CQuestManager::instance().GetEventFlag("guild_disband_delay"));
			return;
		case GERR_ALREADYJOIN:
			pchInviter->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pchInviter->GetLanguage(), "<길드> 상대방이 이미 다른 길드에 속해있습니다."));
			return;
		case GERR_GUILDISFULL:
			pchInviter->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pchInviter->GetLanguage(), "<길드> 최대 길드원 수를 초과했습니다."));
			return;
		case GERR_GUILD_IS_IN_WAR:
			pchInviter->ChatPacket( CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pchInviter->GetLanguage(), "<길드> 현재 길드가 전쟁 중 입니다."));
			return;
		case GERR_INVITE_LIMIT:
			pchInviter->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pchInviter->GetLanguage(), "<길드> 현재 신규 가입 제한 상태 입니다."));
			return;
...
void CGuild::InviteAccept( LPCHARACTER pchInvitee )
{
	...
	switch ( errcode )
	{
		case GERR_NONE:
			break;
		case GERR_WITHDRAWPENALTY:
			pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pchInvitee->GetLanguage(), "<길드> 탈퇴한 후 %d일이 지나지 않은 사람은 길드에 초대할 수 없습니다."), quest::CQuestManager::instance().GetEventFlag("guild_withdraw_delay"));
			return;
		case GERR_COMMISSIONPENALTY:
			pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pchInvitee->GetLanguage(), "<길드> 길드를 해산한 지 %d일이 지나지 않은 사람은 길드에 초대할 수 없습니다."), quest::CQuestManager::instance().GetEventFlag("guild_disband_delay"));
			return;
		case GERR_ALREADYJOIN:
			pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pchInvitee->GetLanguage(), "<길드> 상대방이 이미 다른 길드에 속해있습니다."));
			return;
		case GERR_GUILDISFULL:
			pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pchInvitee->GetLanguage(), "<길드> 최대 길드원 수를 초과했습니다."));
			return;
		case GERR_GUILD_IS_IN_WAR:
			pchInvitee->ChatPacket( CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pchInvitee->GetLanguage(), "<길드> 현재 길드가 전쟁 중 입니다."));
			return;
		case GERR_INVITE_LIMIT:
			pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pchInvitee->GetLanguage(), "<길드> 현재 신규 가입 제한 상태 입니다."));
			return;
