...
namespace quest
{
	...
	int monarch_bless(lua_State * L)
	{
		...
		if (false==ch->IsMonarch())
		{
			if (!ch->IsGM())
			{
				ch->ChatPacket(CHAT_TYPE_INFO ,LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "군주의 자격을 가지고 있지 않습니다"));
		...
		if (CMonarch::instance().HealMyEmpire(ch, HealPrice))
		{
			...
			ch->ChatPacket(CHAT_TYPE_INFO ,LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "군주의 축복을 사용하였습니다."));
		}
	...
	int monarch_powerup(lua_State * L)
	{
		...
		if (!CMonarch::instance().IsMoneyOk(money_need, ch->GetEmpire()))
		{
			...
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "국고에 돈이 부족합니다. 현재 : %u 필요금액 : %u"), NationMoney, money_need);
			return 0;
		}
		...
		if (!CMonarch::instance().CheckPowerUpCT(ch->GetEmpire()))
		{
			...
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "쿨타임 적용중  %d 후 사용가능"), next_sec);
			return 0;
		}
	...
	int spawn_mob(lua_State * L)
	{
...

		{
			...
			if (11505 == mob_vnum)	// 황금두꺼비
			{
				...
				{
					int NationMoney = CMonarch::instance().GetMoney(ch->GetEmpire());
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "국고에 돈이 부족합니다. 현재 : %u 필요금액 : %u"), NationMoney, CASTLE_FROG_PRICE);
					return 0;
				}
	...
	int spawn_guard(lua_State * L)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "성에서만 사용할 수 있는 기능입니다."));
			return 0;
		}
	...
	int frog_to_empire_money(lua_State * L)
	{
		...
		if (castle_frog_to_empire_money(ch))
		{
			...
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "TEST : 황금두꺼비가 국고로 환원되었습니다."));
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "TEST : 현재 국고 : %d"), empire_money);
			castle_save();
			return 1;
		}
		else
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "TEST : 황금두꺼비를 국고로 환원할 수 없습니다."));
			return 0;
		}
	}
		...
		if (!tch)
		{
			...
			if (pkCCI)
			{
				...
				else
				{
					...
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "%s 에게로 이동합니다"), name.c_str());
	...
	int monarch_transfer(lua_State * L)
	{
		...
		if (!tch)
		{
			...
			if (pkCCI)
			{
				...
				if (pkCCI->bChannel != g_bChannel)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "%s 님은 %d 채널에 접속 중 입니다. (현재 채널: %d)"), name.c_str(), pkCCI->bChannel, g_bChannel);
					return 0;
				}
				...
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "%s 님을 소환하였습니다."), name.c_str());
	...
	int monarch_transfer2(lua_State* L)
	{
		...
		{
			int NationMoney = CMonarch::instance().GetMoney(ch->GetEmpire());
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "국고에 돈이 부족합니다. 현재 : %u 필요금액 : %u"), NationMoney, ciTransferCost);
			return 0;
		}
		...
		if (pTargetChar == NULL)
		{
			...
			if (pCCI != NULL)
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "%s 님은 %d 채널에 접속중입니다. (현재 채널: %d)"),
				...
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "소환 요청을 보냈습니다"));
		...
		else
		{
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "%s 님은 근처에 있습니다"), pTargetChar->GetName());
				return 0;
			}
