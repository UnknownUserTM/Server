...
int DESC::ProcessInput()
{
	...
	else
	{
		...
		if (iSizeBuffer > 0)
		{
		//	TEMP_BUFFER	tempbuf; //20160701
		//	LPBUFFER lpBufferDecrypt = tempbuf.getptr(); //20160701
			LPBUFFER lpBufferDecrypt = buffer_new(iSizeBuffer);
		//	buffer_adjust_size(lpBufferDecrypt, iSizeBuffer); //20160701
...
EVENTFUNC(disconnect_event)
{
	...
	LPDESC d = info->desc;
	if (info->ban) {
		DBManager::instance().DirectQuery("UPDATE account.account SET status = 'BOT' WHERE login = '%s'", d->GetAccountTable().login);
	}
...
bool DESC::DelayedDisconnect(int iSec, bool ban)
{
...
void DESC::DisconnectOfSameLogin()
{
	if (GetCharacter())
	{
		...
		GetCharacter()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(GetCharacter()->GetLanguage(), "다른 컴퓨터에서 로그인 하여 접속을 종료 합니다."));
		DelayedDisconnect(5);
	}
	else
	{
...
const char * DESC::GetLanguage()
{
	return m_accountTable.szLanguage;
}
