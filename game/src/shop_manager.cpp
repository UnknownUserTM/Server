...
bool CShopManager::StartShopping(LPCHARACTER pkChr, LPCHARACTER pkChrShopKeeper, int iShopVnum)
{
	...
	{
		pkChr->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(pkChr->GetLanguage(), "다른 거래창이 열린상태에서는 상점거래를 할수 가 없습니다."));
		return false;
	}
...
void CShopManager::Buy(LPCHARACTER ch, BYTE pos)
{
	...
	if (0 != g_BuySellTimeLimitValue)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "아직 골드를 버릴 수 없습니다."));
			return;
		}
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "상점과의 거리가 너무 멀어 물건을 살 수 없습니다."));
		return;
	}
...
void CShopManager::Sell(LPCHARACTER ch, BYTE bCell, DWORD bCount)
{
	...
	if (DISTANCE_APPROX(ch->GetX()-ch->GetShopOwner()->GetX(), ch->GetY()-ch->GetShopOwner()->GetY())>2000)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "상점과의 거리가 너무 멀어 물건을 팔 수 없습니다."));
		return;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "착용 중인 아이템은 판매할 수 없습니다."));
		return;
	}
	...
	if (GOLD_MAX <= nTotalMoney)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "20억냥이 초과하여 물품을 팔수 없습니다."));
		return;
	}

	// 20050802.myevan.상점 판매 로그에 아이템 ID 추가
	sys_log(0, "SHOP: SELL: %s item name: %s(x%d):%u price: %llu", ch->GetName(), item->GetName(), bCount, item->GetID(), dwPrice);

	if (iVal > 0)
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT_CONVERT_LANGUAGE(ch->GetLanguage(), "판매금액의 %d %% 가 세금으로 나가게됩니다"), iVal);
